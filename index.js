...
function flushQueue(session, queued) {
  ...
  // Queue-entry lifecycle → frame lifecycle (#493 blocker 2): the whole queue
  // just flushed (or was dropped with the session dead), so every outstanding
  // "📨 Queued" tile is resolved. Tell the journal router to mark this convo's
  // live queue-action frames resolved — they become idempotent no-ops instead
  // of lingering "live" forever, which is what keeps the live set bounded to
  // currently-actionable tiles without ever recency-evicting a live one. NOT
  // reached on the Codex-busy branch above (it re-queues and returns early —
  // those tiles are still actionable). resolveQueueActionFrames tolerates a
  // missing convo id / journal-disabled build.
  const flushedConvoId = journalConvoIdFor(session);
  if (flushedConvoId && journalInputConsumer?.resolveQueueActionFrames) {
    journalInputConsumer.resolveQueueActionFrames(flushedConvoId);
  }
}
...
function journalOnPromptReply(session, answer, { username }) {
  ...
  // rather than being swallowed as a queue control. The router has already made
  // this idempotent (a resolved tile's repeat tap never reaches here), so the
  // shared handler runs at most once per tap. Run the SAME extracted
