...
- `agent_boxes` lists the user's boxes — this one included, marked "this box" — with recent folders, activity, and usage limits so you can find spare capacity or hand over on the same machine; `agent_session_start` asks the user's consent to seed a task on one of them — the outcome, like everything else here, arrives as a later turn. The new session is detached by default: it does its task and does not report back (a spawn is a clean break; the user reads its transcript directly). Pass `link: true` only when you need its results in a chat room.
...
- Base URL: the **bridge-local journal proxy** on the loopback API — `http://127.0.0.1:$MATRON_BRIDGE_API_PORT/journal`. The bridge injects the journal credential server-side, so you never hold, read, or print a journal token. (`MATRON_BRIDGE_API_PORT` is in your env.) Do not call the remote journal host directly — you have no token for it, by design.
- Auth: send the loopback capability header `X-Matron-Journal-Proxy-Token: $MATRON_JOURNAL_PROXY_TOKEN` on every proxy request (`MATRON_JOURNAL_PROXY_TOKEN` is in your env). It is a low-privilege capability — only the two read routes below, never `/snapshot`, `/roster`, `/items` or writes — but still don't print it: read it inside the request (`-H "X-Matron-Journal-Proxy-Token: $MATRON_JOURNAL_PROXY_TOKEN"`) and never use `curl -v`/`--trace`, which print request headers.
...
- Use `curl` (its default `User-Agent` is fine); the proxy is plain loopback HTTP, so no Cloudflare/Browser-Integrity concern applies to it. Pace request bursts anyway — the journal's own rate limiter still applies to the bridge's forwarded requests and answers 403 across the board for a while once tripped.
