...
describe('createJournalPublisher', () => {
  ...
  it('fetchConvoMessages: GETs /convo/:id/messages with before_seq/limit + Bearer auth and returns the events array', async () => {
    const events = [{ seq: 12, convo_id: 'c-1', type: 'prompt', payload: { options: [{ id: 'model-sonnet', value: 'model:sonnet' }] } }];
    const httpServer = await startFakeHttpServer((entry, res) => {
      if (entry.url.startsWith('/convo/c-1/messages')) {
        res.writeHead(200, { 'content-type': 'application/json' });
    ...
    const result = await pub.fetchConvoMessages('c-1', { beforeSeq: 13, limit: 1 });
    expect(result).toEqual(events);

    const reqs = httpServer.received.filter(r => r.url && r.url.startsWith('/convo/c-1/messages'));
    expect(reqs.length).toBe(1);
    expect(reqs[0].method).toBe('GET');
    expect(reqs[0].headers['authorization']).toBe('Bearer tok-xyz');
    expect(reqs[0].url).toContain('before_seq=13');
    expect(reqs[0].url).toContain('limit=1');

    pub.close();
    await httpServer.close();
  });

  it('fetchConvoMessages: a non-2xx response resolves null, never throws', async () => {
    const httpServer = await startFakeHttpServer((_entry, res) => { res.writeHead(404); res.end(); });
    const wsUrl = `ws://127.0.0.1:${httpServer.port}/ws`;
    const pub = createJournalPublisher({ url: wsUrl, token: 'tok', log: silentLog, ...FAST_BACKOFF });

    expect(await pub.fetchConvoMessages('c-x', { beforeSeq: 5, limit: 1 })).toBeNull();

    pub.close();
    await httpServer.close();
  });

  it('fetchConvoMessages: a body without an events array resolves null', async () => {
    const httpServer = await startFakeHttpServer((_entry, res) => {
      res.writeHead(200, { 'content-type': 'application/json' });
  ...
  it('fetchConvoMessages: a blank convo_id resolves null with no HTTP call', async () => {
    const httpServer = await startFakeHttpServer();
    const wsUrl = `ws://127.0.0.1:${httpServer.port}/ws`;
    const pub = createJournalPublisher({ url: wsUrl, token: 'tok', log: silentLog, ...FAST_BACKOFF });

    expect(await pub.fetchConvoMessages('', { beforeSeq: 5, limit: 1 })).toBeNull();
    expect(httpServer.received.filter(r => r.url && r.url.startsWith('/convo')).length).toBe(0);

    pub.close();
    await httpServer.close();
  });

  it('fetchConvoMessages: a disabled (unconfigured) publisher returns null', () => {
    const pub = createJournalPublisher({ url: null, token: null, log: silentLog });
    expect(pub.fetchConvoMessages('anything', {})).toBeNull();
  });
});
