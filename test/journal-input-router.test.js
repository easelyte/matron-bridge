...
describe('createJournalInputConsumer — queue-action replies bypass the staleness guard', () => {
  ...
  it('a queue-action frame is remembered across taps (a second tap still routes as a queue control — not single-use)', () => {
    const deps = makeDeps();
    const consumer = createJournalInputConsumer(deps);
    consumer(queueActionFrame(12));
    consumer(answerableFrame(10));
    consumer(reply(12, 'interrupt'));
    consumer(reply(12, 'interrupt')); // second tap on the same tile still recognized
    expect(deps.routePromptReply).toHaveBeenCalledTimes(2);
    expect(deps.routePromptReply.mock.calls[0][1].queue_action).toBe(true);
    expect(deps.routePromptReply.mock.calls[1][1].queue_action).toBe(true);
  });
