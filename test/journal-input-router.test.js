...
// #492: durable picker provenance across a bridge restart. The in-memory
// pickerFramesByConvo record is process-local — a picker card published before
// a restart is gone from the map afterwards, so its reply used to fall through
// to ordinary prompt handling ("Nothing to answer"). The canonical fix resolves
// the frame from the journal event itself via an injected async seam
// (resolveTargetPromptPayload), so provenance survives the restart with no
// durable shadow of its own (single-source-of-truth: the journal event).
describe('createJournalInputConsumer — durable picker provenance (canonical journal lookup, #492)', () => {
  function makeDeps(overrides = {}) {
    return {
      isControlConvo: () => false,
      handleControlCommand: vi.fn(),
  ...
  // A picker `prompt` event's payload as it lives in the journal store.
  const pickerPayload = (opts) => ({ question: 'pick', mode: 'pick_one', options: opts });
  const pickerReply = (targetSeq, choice) => baseFrame({
    seq: 100, type: 'prompt_reply',
    payload: { target_seq: targetSeq, choice, text: null },
  });
  ...
  it('resolves a picker tap from the canonical journal event when the in-memory frame is gone (post-restart)', async () => {
    // A fresh consumer has NO in-memory record for seq 12 (simulating a restart
    // after the card was published). The seam returns the canonical frame.
    const resolveTargetPromptPayload = vi.fn(async (convoId, targetSeq) =>
      (convoId === 'convo-1' && targetSeq === 12)
        ? pickerPayload([{ id: 'model-sonnet', value: 'model:sonnet' }])
        : null);
    const deps = makeDeps({ resolveTargetPromptPayload });
    const consumer = createJournalInputConsumer(deps);

    consumer(pickerReply(12, 'model:sonnet'));
    await flush();

    expect(resolveTargetPromptPayload).toHaveBeenCalledWith('convo-1', 12);
    expect(deps.routePromptReply).toHaveBeenCalledTimes(1);
    expect(deps.routePromptReply.mock.calls[0][1]).toMatchObject({ target_seq: 12, choice: 'model:sonnet', picker: true });
    expect(deps.noticeStalePromptReply).not.toHaveBeenCalled();
  });

  it('does NOT invoke the canonical lookup for an ordinary (non-picker-shaped) reply — no HTTP on the common path', async () => {
    const resolveTargetPromptPayload = vi.fn(async () => null);
    const deps = makeDeps({ resolveTargetPromptPayload });
    const consumer = createJournalInputConsumer(deps);

    consumer(pickerReply(7, 'opt_a')); // a normal answer choice, not model:/effort:/mode:
    await flush();

    expect(resolveTargetPromptPayload).not.toHaveBeenCalled();
    // Falls through to ordinary routing (no staleness record → accepted).
    expect(deps.routePromptReply).toHaveBeenCalledTimes(1);
    expect(deps.routePromptReply.mock.calls[0][1]).toMatchObject({ target_seq: 7, choice: 'opt_a' });
    expect(deps.routePromptReply.mock.calls[0][1].picker).toBeUndefined();
  });

  it('falls through to ordinary routing when the canonical event is not a picker frame', async () => {
    // The seam finds an answerable prompt at that seq, not a picker — a genuine
    // answer whose label merely looks like a picker value must NOT be hijacked.
    const resolveTargetPromptPayload = vi.fn(async () =>
      ({ question: 'Which model do you prefer?', mode: 'pick_one', options: [{ id: 'opt_a', label: 'model:sonnet' }] }));
    const deps = makeDeps({ resolveTargetPromptPayload });
    const consumer = createJournalInputConsumer(deps);
    ...
    expect(resolveTargetPromptPayload).toHaveBeenCalledTimes(1);
    expect(deps.routePromptReply).toHaveBeenCalledTimes(1);
    expect(deps.routePromptReply.mock.calls[0][1].picker).toBeUndefined();
  });

  it('a canonical frame does not authorize a value it never offered (cross-namespace)', async () => {
    const resolveTargetPromptPayload = vi.fn(async () =>
      pickerPayload([{ id: 'mode-print', value: 'mode:print' }])); // a MODE frame
    const deps = makeDeps({ resolveTargetPromptPayload });
    const consumer = createJournalInputConsumer(deps);
    ...
    // Not dispatched as a picker; falls through to ordinary routing.
    expect(deps.routePromptReply).toHaveBeenCalledTimes(1);
    expect(deps.routePromptReply.mock.calls[0][1].picker).toBeUndefined();
  });

  it('falls through to ordinary routing (and never throws) when the seam returns null or rejects', async () => {
    const resolveTargetPromptPayload = vi.fn()
      .mockResolvedValueOnce(null)
      .mockRejectedValueOnce(new Error('journal unreachable'));
    const deps = makeDeps({ resolveTargetPromptPayload });
    const consumer = createJournalInputConsumer(deps);

    consumer(pickerReply(12, 'model:sonnet')); // seam → null
    consumer(pickerReply(13, 'effort:high'));  // seam → throws
    await flush();

    expect(deps.routePromptReply).toHaveBeenCalledTimes(2);
    expect(deps.routePromptReply.mock.calls.every((c) => c[1].picker === undefined)).toBe(true);
  });

  it('is single-use across the canonical path — a duplicate reply is not dispatched twice', async () => {
    const resolveTargetPromptPayload = vi.fn(async () =>
      pickerPayload([{ id: 'model-sonnet', value: 'model:sonnet' }]));
    const deps = makeDeps({ resolveTargetPromptPayload });
    const consumer = createJournalInputConsumer(deps);

    consumer(pickerReply(12, 'model:sonnet')); // first tap → canonical dispatch, frame consumed
    await flush();
    consumer(pickerReply(12, 'model:sonnet')); // duplicate → must not re-resolve + re-dispatch
    await flush();

    const pickerDispatches = deps.routePromptReply.mock.calls.filter((c) => c[1].picker === true);
    expect(pickerDispatches).toHaveLength(1);
  });

  it('without the seam wired, behavior is unchanged — a lost-frame picker reply falls through', async () => {
    const deps = makeDeps(); // no resolveTargetPromptPayload
    const consumer = createJournalInputConsumer(deps);
