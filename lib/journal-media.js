...
export function createJournalMediaRouter({
  ...
  // blocks[] or { blocks, ivHandled }. The save-to-disk + inline-block builder
  // shared with the Matrix media path; ivHandled means the iv branch already
  // folded the caption into its annotation.
  buildSavedBlocks,
  ...
  async function routeOne(session, media, ctx = {}) {
    try {
      ...
      const built = buildSavedBlocks(session, {
        buffer, mime, isImage: type === 'image', name, dims, caption: caption || null,
      });
      const blocks = Array.isArray(built)
        ? built
        : (built && Array.isArray(built.blocks) ? built.blocks : []);
