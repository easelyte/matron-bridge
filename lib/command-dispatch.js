...
// /restart's immediate-restart flag: with '--force' a busy session restarts
// on the spot (killing the in-flight turn, the historical behavior); without
// it the restart is deferred to the turn-end seams so e.g. a running
// /compact isn't cancelled. Mobile keyboards auto-correct a leading `--`
// into an em/en dash, so the same leading-unicode-dash normalization
// extractMcpExtraFlags and extractAgentFlag apply is used here; the match is
// case-insensitive for the same phone-keyboard reasons. Non-force tokens are
// preserved VERBATIM (dashes un-normalized) so the downstream extras/agent
// parsers see exactly what the user typed.
const LEADING_UNICODE_DASHES = /^[‐‑‒–—―]+/;
export function extractForceFlag(tokens = []) {
  let force = false;
  const rest = [];
  for (const original of tokens) {
    const normalized = String(original).replace(LEADING_UNICODE_DASHES, '--').toLowerCase();
    if (normalized === '--force') force = true;
    else rest.push(original);
  }
  return { force, rest };
}
...
export const JOURNAL_CONTROL_HELP_NOTE =
  ...
  '- Session-scoped commands (/status, /stop, /restart, /show, /working, /agent, /switch, /mcp, /model, /mode, /effort, /cost, /usage, /tools, /login, /logout, /timer, /sleep) ' +
