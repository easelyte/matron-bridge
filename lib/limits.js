...
export function allMembersAllowed(memberIds, allowedIds, botUserId) {
  ...
  const nonBotMembers = memberIds.filter((m) => m !== botUserId);
  return nonBotMembers.length > 0 && nonBotMembers.every((m) => allowedIds.includes(m));
}

async function isRoomSafeForLimits(client, roomId) {
  try {
    await client.getRoomStateEvent(roomId, 'm.room.encryption', '');
    return true;
  } catch {
    // Missing encryption state is acceptable only if history visibility is private.
  }

  try {
    const event = await client.getRoomStateEvent(roomId, 'm.room.history_visibility', '');
    return event?.history_visibility === 'joined';
  } catch {
    return false;
  }
}

export function makeMembershipGate({ client, allowedIds, botUserId, allowAny = false, log = console }) {
  return async function gate(roomId) {
    if (!allowedIds || allowedIds.length === 0) return allowAny && isRoomSafeForLimits(client, roomId);
    try {
      const ids = await client.getJoinedRoomMembers(roomId);
      if (!allMembersAllowed(ids, allowedIds, botUserId)) return false;
      return isRoomSafeForLimits(client, roomId);
    } catch {
      log.warn?.(`[limits] membership lookup failed for ${roomId} - failing closed`);
      return false;
    }
  };
}
