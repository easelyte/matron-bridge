...
// (design §10.1), so phase 1 ships the write without changing peer-visible
// behaviour: opt in with AGENT_ROSTER_SUMMARY_ENABLED=1 once the call is made.
//
// Scope, stated honestly: this gates THIS TOOL's output, and is not an
// authorization boundary. Spawned sessions inherit the bridge environment
// (JOURNAL_TOKEN included) and the journal's GET /roster hands the stored
// summary to any agent token, so an agent that bypasses the tool can still
// read it. Enforcing it properly means scoping journal credentials out of
// child environments and/or a server-side per-device capability — separate,
// unshipped work. SUMMARY_JOURNAL_PUBLISH=0 is the switch that actually
// prevents the text from being stored.
