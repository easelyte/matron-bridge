...
import { summaryBlocks } from './pinned-summary.js';
...
// it and (since 2026-09-21) is no longer called by session teardown either:
// rooms outlive the claude process, and index.js orphanRoomBinding leaves a
// binding only once its conversation cannot be resumed — each
...
// --- agent_roster summary blurb (loop #554 B4) ---
//
// conversations.summary is documented as a "rolling 2-3 sentence" targeting
// aid for peer agents (matron-journal docs/protocol.md), but phase 1 starts
// writing the bridge's rolling BULLET digest into it — up to 1000 chars and
// 20 bullets. rosterBlurb re-derives the documented short form at the READ
// seam so the peer contract is honoured without a second producer, a second
// column, or a second model call.
const ROSTER_BLURB_BULLETS = 3;
const ROSTER_BLURB_MAX_CHARS = 300;

export function rosterBlurb(text, { maxBullets = ROSTER_BLURB_BULLETS, maxChars = ROSTER_BLURB_MAX_CHARS } = {}) {
  // summaryBlocks, not a bullet-line filter: a wrapped bullet's continuation
  // lines belong to their bullet, and that is exactly where a "…but it is
  // blocked on X" clause lands. Dropping them would hand peers a blurb that
  // reads like an unqualified success. Shared with summaryForJournal so the
  // publish side and the read side agree on what a bullet is. Prose with no
  // markers at all is one block, so it survives rather than blanking.
  const picked = summaryBlocks(text).slice(-maxBullets)
...
// DEFAULT OFF. Populating conversations.summary changes what OTHER agents see
// in agent_roster, from "" (its value for every one of the 2056 live
// conversations today) to a real digest of the operator's session. Whether a
// peer agent should see that is an unanswered product/privacy call
// (design §10.1), so phase 1 ships the write without changing what this tool
// prints: opt in with AGENT_ROSTER_SUMMARY_ENABLED=1 once the call is made.
//
// What this flag is NOT. It gates this tool's output and nothing else. It does
// not gate the journal: GET /roster returns the raw conversations.summary
// column to any caller holding a valid agent token regardless of this setting,
// and rosterBlurb() never runs on that path — a peer hitting the endpoint
// directly reads the full stored digest, not the trimmed blurb. Do not read
// "default off" as "peers cannot see it".
//
// The real boundary is the journal token. Anything with JOURNAL_TOKEN can read
// the column, and that token already reads whole transcripts via /snapshot, so
// the digest is strictly less than what it already grants. Spawned sessions
// inherit the bridge environment, JOURNAL_TOKEN included. A genuine boundary
// means scoping journal credentials out of child environments and/or a
// server-side per-device capability — separate, unshipped work.
// SUMMARY_JOURNAL_PUBLISH=0 is the only switch that keeps the text out of the
// column in the first place.
export function rosterSummaryEnabled(env = process.env) {
  const value = String(env?.AGENT_ROSTER_SUMMARY_ENABLED ?? '').trim().toLowerCase();
    ...
    async roster(data) {
      ...
      return { status: 200, body: {
        ...
        conversations: (r.conversations || []).map((c) => ({
          ...
          // '' unless AGENT_ROSTER_SUMMARY_ENABLED is set — see rosterSummaryEnabled.
          // Evaluated per call, not captured at factory time, so the flag can be
          // flipped by a restart alone.
          summary: rosterSummaryEnabled(env) ? rosterBlurb(c.summary) : '',
          agent_device_id: c.agent_device_id, last_ts: c.last_ts,
