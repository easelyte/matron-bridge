...
export function rosterBlurb(text, { maxBullets = ROSTER_BLURB_BULLETS, maxChars = ROSTER_BLURB_MAX_CHARS } = {}) {
  ...
  // A bridge (or an upstream one) that wrote prose instead of bullets still
  // gets a sensible blurb rather than an empty string.
  const picked = (bullets.length ? bullets : lines).slice(-maxBullets)
