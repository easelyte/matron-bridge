...
export function createAgentChatHandlers({ sessions, publisher, rooms, invites, awaitRoomMessage = async () => null, pendingPeerFor = () => null, clearPendingPeer = () => {}, journalConvoIdFor = () => null, serverLabel = '', log = console } = {}) {
  ...
  return {
    ...
    async agentMessage(data) {
      ...
      // sessionKey is the invoking MCP process's BRIDGE_ROOM_ID, carried as
      // roomId by the loopback wrapper. Never accept model-supplied attribution.
      const outcome = await publisher.sendPeerMessage({
        targetConvo,
        fromConvo: sessionKey,
        body,
      });
