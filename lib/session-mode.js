...
import { inflightMediaReplaceRefusal } from './session-replace-guard.js';
...
// Precedence: explicit option -> persisted -> box fallback -> undefined
// (Claude Code's own default). `fallback` is MATRON_DEFAULT_MODEL, the
// model a new chat starts on when nobody picked one.
//
// `resumed` (planSessionIdentity's verdict) switches the implicit fallback
// OFF: a --resume of a room that never persisted a model must not gain a
// --model flag, because on a resume that flag is exactly the mechanism that
// changes the conversation's model, and the next persist would lock the
// switch in (Bugbot + CodeRabbit, PR #256). The box default decides FRESH
// starts only; an explicit pick still wins on either path.
//
// The 'default' alias resolves to the fallback when one is configured. The
// New Chat picker's first option is labelled "Default", and on a box whose
// operator declared a default model that word means the box's default, not
// Claude's — otherwise the picker's preselected choice would be the one
// value the setting never applies to.
export function resolveModel({ option, persisted, fallback, resumed = false }) {
  const picked = option ?? persisted;
  if (picked === 'default' && fallback) return fallback;
  if (picked !== undefined && picked !== null) return picked;
  return resumed ? undefined : (fallback ?? undefined);
}
...
export function planModeSwitch(session, wantInteractive, { hasInflightMedia = false } = {}) {
  ...
  // A /mode switch recreates the session (same as /switch / /restart). Refuse
  // while a journal-media attachment for this conversation is still being
  // prepared — the teardown would make the media router drop it at its
  // delivery-time canonicality guard. Checked last, so it only blocks an
  // otherwise-allowable switch; age-bounded upstream so it can't wedge forever.
  const mediaRefusal = inflightMediaReplaceRefusal(hasInflightMedia);
  if (mediaRefusal.refuse) {
    return { ok: false, message: mediaRefusal.message };
  }
