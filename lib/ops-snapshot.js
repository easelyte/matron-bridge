...
// (`python3 -W ignore:<secret> app.py`, `node --token x app.js`): the scan
// cannot know which flags take one, so an argument is only named when its
// shape proves it is a script — a script-file extension, or (right after the
// interpreter, with no flags in between) a plain path such as
// `node /usr/bin/codex`, or python's `-m <module>`.
const SCRIPT_EXT = /^[\w.@+-]{1,60}\.(js|mjs|cjs|ts|mts|cts|jsx|tsx|py|pyw|sh|bash|zsh)$/;
const PLAIN_PATH = /^[\w./@+-]+$/;
...
export function processName(argv, comm = null) {
  ...
  if (INTERPRETER.test(base)) {
    ...
    for (let i = 1; i < args.length; i++) {
      ...
      if (a.startsWith('-')) { prevWasFlag = true; continue; }
      if ((base === 'bun' || base === 'deno') && RUNNER_SUBCOMMANDS.has(a)) continue;
      const script = path.basename(a);
      if (SCRIPT_EXT.test(script) || (i === 1 && PLAIN_PATH.test(a) && script)) {
        name = `${base} ${stripControl(script)}`;
        break;
      }
      // Possibly a flag's operand: keep looking for a provable script. A bare
      // non-script positional that follows no flag is where the program's
      // own arguments begin, so stop there.
