...
// A script argument we are willing to name. Flag OPERANDS are the danger
// (`python3 -W ignore:<secret> app.py`, `python3 -X <secret>.py`): a scan
// that guesses which flags take an operand leaks the ones it guesses wrong.
// So flags are allow-listed per interpreter family as KNOWN to take no
// separate operand (or to carry it inline as `--flag=value`); the first
// unknown flag ends the scan with the bare interpreter name. The positional
// after only-known flags is the script; it is named only when it is shaped
// like a path, and python's `-m <module>` names the module.
const SCRIPT_PATH = /^[\w./@+-]+$/;
const PY_MODULE = /^[A-Za-z_][\w.]{0,60}$/;
const NODE_BARE_FLAGS = new Set([
  '--no-warnings', '--enable-source-maps', '--expose-gc', '--trace-warnings', '--trace-deprecation',
  '--no-deprecation', '--inspect', '--inspect-brk', '--preserve-symlinks', '--preserve-symlinks-main',
  '--experimental-vm-modules', '--experimental-strip-types', '--no-experimental-fetch', '--watch',
  '--hot', '--smol', '-A', '-q', '--quiet', '--unstable',
]);

function isOperandFreeFlag(base, a) {
  if (/^(bash|sh|dash|zsh)$/.test(base)) return /^[-+][abefhkmnptuvxBCEHPT]+$/.test(a) || ['--login', '--norc', '--noprofile', '--posix'].includes(a);
  if (base.startsWith('python')) return /^-[bBdEiIOPqRsSuvx]+$/.test(a);
  // node / bun / deno: `--flag=value` carries its operand inline.
  if (/^--[\w-]+=/.test(a)) return true;
  if (base === 'deno' && /^--allow-[\w-]+$/.test(a)) return true;
  return NODE_BARE_FLAGS.has(a);
}
...
export function processName(argv, comm = null) {
  ...
  if (INTERPRETER.test(base)) {
    for (let i = 1; i < args.length; i++) {
      ...
      if (base.startsWith('python') && a === '-m') {
        const mod = args[i + 1];
        if (typeof mod === 'string' && PY_MODULE.test(mod)) name = `${base} ${mod}`;
        break;
      }
      if (a.startsWith('-') || (a.startsWith('+') && /^(bash|sh|dash|zsh)$/.test(base))) {
        if (isOperandFreeFlag(base, a)) continue;
        break; // unknown flag: its operand may be next — name nothing
      }
      if ((base === 'bun' || base === 'deno') && RUNNER_SUBCOMMANDS.has(a)) continue;
      const script = path.basename(a);
      if (SCRIPT_PATH.test(a) && script && script.length <= 60) name = `${base} ${script}`;
      break;
    }
  }
...
// Kernel page size, for the /proc/<pid>/stat RSS estimate used to rank
// processes (Node exposes no sysconf). Derived from this process's own
// VmRSS (kB) / stat rss (pages), snapped to a known size; 4 KiB when either
// read fails. 64 KiB-page hosts (some arm64/ppc64le) would otherwise have
// every process under-estimated 16x and filtered out.
const KNOWN_PAGE_SIZES = [4096, 16384, 65536];
export async function detectPageSize(readText) {
  try {
    const st = parseProcPidStat(await readText('/proc/self/stat'));
    const { rssBytes } = parseStatus(await readText('/proc/self/status'));
    if (!st || !(st.rssPages > 0) || !(rssBytes > 0)) return 4096;
    const ratio = rssBytes / st.rssPages;
    return KNOWN_PAGE_SIZES.reduce((best, size) => (Math.abs(Math.log2(size / ratio)) < Math.abs(Math.log2(best / ratio)) ? size : best));
  } catch {
    return 4096;
  }
}

// Run fn over items with at most `limit` in flight: a process-heavy host
// must not turn one Ops page load into thousands of simultaneous reads.
...
async function sampleProcesses(pids, readText) {
  const out = new Map();
  await mapLimit(pids, PROC_READ_CONCURRENCY, async (pid) => {
    try {
...
export async function readHostSection({
  ...
  const pageBytes = Number.isInteger(pageSize) && pageSize > 0 ? pageSize : await detectPageSize(readText);
  ...
  // Whole-token numeric parse: a garbage-suffixed token is a failed probe.
  const num = (tok) => (typeof tok === 'string' && /^\d+(\.\d+)?$/.test(tok) ? Number(tok) : NaN);
  const uptime = Math.floor(num(String(uptimeText).trim().split(/\s+/)[0]));
  const load = String(loadText).trim().split(/\s+/).slice(0, 3).map(num);
  const mem = parseMeminfo(memText);
  // Required probes: a failed read answers unavailable rather than shipping
  // zeros that read as real measurements. (/etc/passwd is optional: user
  // simply becomes null.)
  if (!Number.isFinite(uptime) || uptime < 0) throw new OpsError('unavailable', 'could not read /proc/uptime');
  if (load.length !== 3 || !load.every(Number.isFinite)) throw new OpsError('unavailable', 'could not read /proc/loadavg');
  if (!(mem.MemTotal > 0) || !Number.isFinite(mem.MemAvailable)) throw new OpsError('unavailable', 'could not read /proc/meminfo');
  ...
  await mapLimit(procs1.keys(), PROC_READ_CONCURRENCY, async (pid) => {
    try {
  ...
  for (const [pid, st] of procs1) {
    ...
    rows.push({ pid, st, argv, rssEstimate: st.rssPages * pageBytes, cpu_pct: pct });
  }
  ...
  // §2.1 requires the disk object; a failed probe (default workdir gone,
  // statfs error) is a host-section failure, not a silently different shape.
  let disk;
  try { disk = getDisk() || null; } catch { disk = null; }
  if (!disk) throw new OpsError('unavailable', 'disk probe failed');
  let live;
  try {
