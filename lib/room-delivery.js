...
export function createRoomDelivery({ isBusy, injectTurn, formatter = formatRoomDelivery, log = console } = {}) {
  ...
  return {
    ...
    flush(session, sessionKey) {
      ...
      return tryInject(session, formatter(list, { coalesced: true, droppedByRoom }), roomIds, list.length);
    },
