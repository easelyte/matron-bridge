...
//
// The item id is part of the key so two comments with identical bodies on
// DIFFERENT items in the same bucket do NOT collide (they must stay distinct);
// for item_create the id is absent and kind+title+body is the content signature.
export function itemIdemKey({ op, roomId, kind, title, body, id, now = Date.now() }) {
  const bucket = Math.floor(Number(now) / BUCKET_MS);
  const parts = [op, roomId, kind, title, body, id].map((v) => (typeof v === 'string' ? v : ''));
  return createHash('sha256').update(`${parts.join('|')}|${bucket}`).digest('hex');
}
