...
// in attachments/labels/links/awaiting/position/supersedes/actions — or two comments that
...
export function itemIdemKey({ op, roomId, args = {}, now = Date.now() }) {
  ...
  const canonical = JSON.stringify([
    ...
    // One-tap reply labels (upstream item actions): a refile that only adds or
    // changes buttons is a different mutation, not a replay.
    nn(args.actions),
  ]);
