...
// `mode` (optional) is applied to the TEMP file, so the rename carries it onto
// the target: a store that must not be world-readable is never world-readable,
// not even for the instant between rename and a chmod. chmodSync afterwards
// covers the case where a temp from a previous run survived and kept its old
// (looser) mode, which writeFileSync's `mode` would not fix on its own.
export function atomicWriteFileSync(file, data, { fs = nodeFs, mode } = {}) {
  ...
  try {
    ...
    fs.writeFileSync(tmpFile, data, mode === undefined ? { flush: true } : { flush: true, mode });
