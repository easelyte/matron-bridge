...
// createdAt, holdAwake?, source? }] }. IDs are bridge-global and monotonic
...
export function createTimerStore({ load, save, now, setTimer, clearTimer, onFire, log = () => {} }) {
  ...
  return {
    ...
    add({ convoId, roomId, text, delayMs, holdAwake = false, source = undefined, requirePersist = false }) {
      const record = {
        id: nextId++,
        convoId,
        roomId: roomId || null,
...
export function keepAwakeMarker(timers) {
  const holds = (timers || []).filter(t => t && t.holdAwake && Number.isFinite(t.fireAt));
