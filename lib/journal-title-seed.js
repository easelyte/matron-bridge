...
// UNTRUSTED transcript content, so extraction is:
//   - line-anchored (/^REPO:...$/im, first match) — a mid-line `REPO:` echoed
//     inside TITLE/prose cannot hijack the value;
//   - sentinel-guarded — `unknown`/`none`/etc. mean "no override";
//   - sanitized — tag delimiters + stray angle brackets + the `·` title
//     separator are stripped (content preserved; CodeQL parity with
//     applyFallbackTitle) so nothing forges markup or extra title segments;
//   - capped at REPO_LABEL_MAX like every other repo label.
// Returns null when absent, a sentinel, or empty after cleaning.
export function extractRepoOverride(text) {
  if (typeof text !== 'string') return null;
  // Horizontal whitespace only ([^\S\r\n]) + a line-bounded capture ([^\r\n]*)
  // so a blank `REPO:` line does NOT let `\s*`/`.+` cross the newline and
  // swallow the following SUMMARY:/NEW: field as the repo. Empty capture →
  // caught by the !raw guard below → workdir fallback.
...
export function formatRoomTitle({ serverLabel, workdir, text, defaultWorkdir, repo }) {
  ...
  const base = `${serverLabel} · ${repoSegment}`;
...
export function applyFallbackTitle(session, { serverLabel, updateRoomName, workdir, defaultWorkdir }) {
  ...
  for (const m of history) {
    ...
    updateRoomName(session.roomId, formatRoomTitle({ serverLabel, workdir, text: clean, defaultWorkdir }));
    return true;
  }
