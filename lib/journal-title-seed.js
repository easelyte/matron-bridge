...
export function applyFallbackTitle(session, { serverLabel, updateRoomName, workdir, defaultWorkdir, repo }) {
  if (session._fallbackTitleApplied) {
    // ONE upgrade is allowed: a fallback first applied with NO repo signal (the
    // common case — the flush that names a short session fires before its first
    // tool result commits activity) may be re-applied once inference yields a
    // repo (F1r2). Guarded so it cannot clobber a better title: skip if we
    // already titled with a repo, if there is still no repo, or if the current
    // title is no longer the one WE set (a codex summary pass has since won).
    if (session._fallbackRepoInferred || !repo) return false;
  ...
  for (const m of history) {
    ...
    const title = formatRoomTitle({ serverLabel, workdir, text: clean, defaultWorkdir, repo });
    session._fallbackTitleApplied = true;
    session._fallbackRepoInferred = !!repo;
