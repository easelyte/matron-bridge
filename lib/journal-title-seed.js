...
export function repoLabel(workdir, { defaultWorkdir }) {
  // A workdir-less session (e.g. a spawned session named from its task, not a
  // cwd) has no repo segment — degrade to empty rather than throw in
  // path.resolve. formatRoomTitle then titles from the text alone.
...
export function formatRoomTitle({ workdir, text, defaultWorkdir, repo }) {
  ...
  // No repo segment (workdir-less session): title from the text alone, no
  // dangling " · " separator.
  if (!base) return text ? truncateWithEllipsis(text, FALLBACK_TITLE_MAX) : '';
