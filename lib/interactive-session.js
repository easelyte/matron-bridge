...
import { transcriptPathFor } from './transcript-dir.js';
...
export class InteractiveSession extends EventEmitter {
  ...
  // ticked yet).
  drainTranscript() {
    // Stop-hook callers do not consume asynchronous drain completion. A zero
    // window performs the strict synchronous tick without arming per-turn
    // interval/timeout pairs; drain itself remains non-rejecting.
    this.tail.drain({ windowMs: 0 });
  }
