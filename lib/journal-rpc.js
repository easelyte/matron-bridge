...
export function createRpcRequestHandler({
  ...
  // Ops page backend (loop #542 phase B, lib/ops-snapshot.js
  // createOpsSnapshot): async (section) -> {ok, result} | {ok:false, error}.
  // Unwired -> ops_snapshot answers unknown_method, exactly like a bridge
  // that predates it, so the web shows "needs bridge update".
  opsSnapshot = null,
  ...
  const handlers = {
    ...
    start(request) {
      ...
      // A duplicate that lands while the first start is still awaiting its
      // mission join must not be told "ok" yet: the opening turn may still be
      // refused and the child stopped. Answer it with whatever the first
      // start finally answers.
      if (cached && cached.pending) {
        return cached.pending.then((r) => respond(request, r.ok, r.body));
      }
      ...
      // What finish() answered, for duplicates parked on a pending start.
      const outcome = { ok: false, body: { code: 'spawn_failed', detail: 'start did not complete' } };
      const reply = (ok, body) => { outcome.ok = ok; outcome.body = body; return respond(request, ok, body); };
      ...
      const finish = (joinOutcome = null) => {
        ...
        if (prompt) {
          ...
          if (!injectTurn || (roomId && (!bindSpawnRoom || !unbindSpawnRoom))) {
            ...
            return reply(false, { code: 'unsupported_mode', detail: roomId ? 'spawn-room wiring absent' : 'spawn wiring absent' });
          }
          try {
            ...
            startDedup.delete(dedupKey);
            try { stopSession(session); } catch { /* best-effort teardown */ }
            return reply(false, { code: 'spawn_failed', detail: e?.message ?? String(e) });
          }
        }
        return reply(true, { convo_id: convoId });
      };
      ...
      // The dedup entry stays PENDING until finish() has answered, so a
      // duplicate in that window waits for the real outcome instead of being
      // answered with a convo that may yet be stopped. Success settles it to
      // the convo id; failure has already dropped it in finish().
      if (missionNum && typeof joinMission === 'function') {
        const entry = {};
        const run = joinSpawnMission(session, missionNum).then(finish);
        entry.pending = run.then(() => outcome, () => outcome);
        startDedup.set(dedupKey, entry);
        run.then(
          () => { if (outcome.ok && startDedup.get(dedupKey) === entry) startDedup.set(dedupKey, convoId); },
          () => { if (startDedup.get(dedupKey) === entry) startDedup.delete(dedupKey); },
        );
        return run;
      }
      return finish();
  ...
  // Read-only host/ops snapshot for the Matron web Ops page (wire contract
  // 2026-09-26 §2). Only `params.section` is honoured; validation, the /proc
  // sampling, the MATRON_OPS_SNAPSHOT_CMD run and the 12,000-byte budget live
  // in lib/ops-snapshot.js.
  //
  // Gating: none added here, deliberately. The journal relay delivers an
  // agent_request only from a CLIENT device of the same user that owns this
  // agent device — the same gate read_file / edit_file / recent_folders rely
  // on. A second, weaker bridge-side check would only suggest otherwise.
  if (typeof opsSnapshot === 'function') {
    handlers.ops_snapshot = async (request) => {
      const params = request.params && typeof request.params === 'object' ? request.params : {};
      const outcome = await opsSnapshot(params.section);
      if (outcome?.ok) respond(request, true, outcome.result);
      else respond(request, false, outcome?.error ?? { code: 'unavailable', detail: 'snapshot failed' });
    };
  }
