...
export function createRpcRequestHandler({
  ...
  const handlers = {
    ...
    start(request) {
      ...
      // Adopt upstream's detached-by-default gate (#282, 2026-09-17): inject the
      // opening turn for any spawn carrying a prompt; the room bind inside is
      // itself guarded on roomId, so a detached spawn (link:false) still gets its
      // task as the opening turn but no room to report into.
      if (prompt) {
