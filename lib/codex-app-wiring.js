...
export function wireCodexAppSession(session, { publisher, convoIdFor, stream, activity, status, notice,
  ...
  function finishChild(id, outcome) {
    ...
    // Clear the running record on LOCAL send completion (onLocalSendComplete = ws.send write
    // callback, not a server commit ack). Same store + reconcile coverage as the subagent
    // finish() path (subagent-convos.js): reconcileStrandedSubagents only retires children under
    // a dead parent, so a finished codex child under a live parent must clear here or it lingers
    // in subagentRunningStore. Reconcile remains the backstop for a lost send once the parent is
    // terminal; the socket-accepted-but-server-uncommitted-under-live-parent residual is the F4
    // note-path residual, out of scope for #754 (would need a server ack — Option A, rejected).
    if (publisher.upsertConvoBestEffort) publisher.upsertConvoBestEffort(id, opts, { onLocalSendComplete: () => runningStore?.remove(id) });
