...
export function createCodexConvoTracker({ publisher, getParentConvoId, log = console } = {}) {
  ...
  function ensureChild(meta) {
    ...
    if (typeof runId !== 'string' || !runId) {
      warn('[codex-convos] missing runId; skipping child');
      return null;
    }
    ...
    const child = {
      ...
      title: typeof meta.label === 'string' && meta.label.trim() ? meta.label : null,
  ...
  return {
    ensureChild(meta) {
      try { return ensureChild(meta); }
        ...
        warn(`[codex-convos] ensureChild failed: ${error.message}`);
        return null;
      }
    },
    ...
    terminalize(runId, outcome) {
      ...
      catch (error) { warn(`[codex-convos] terminalize failed: ${error.message}`); }
    },
