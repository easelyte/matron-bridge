  ...
  function ensureChild(meta) {
    ...
    if (children.size >= childLimit) {
      if (!childLimitNoted) {
        ...
        // publishText is fire-and-forget; its queue retains this idem key for
        // reconnect retries. Separately, watchdog retryPending and T-5.4
        // terminal re-emission are safe under T-3.1's set-once COALESCE.
