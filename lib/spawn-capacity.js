...
// (wire contract 2026-09-26 §1). as_of is the Claude fetch time, or the
// Codex one while the Claude cache is cold. Null when both are cold — the
...
export function buildLimits(cache, codexCache = null) {
  ...
  const as_of = claudeLines.length > 0 ? cache.fetchedAt : codexCache.fetchedAt;
