...
// The tier of a ROOM turn about to be injected, given the batch of room
// messages it carries. Room messages are provenance-mixed: an agent-origin
// message is a coordinating peer turn (peer-coalesced tier, preemptable by a
// priority peer), while a user-origin message is the operator typing into a
// room and must stay operator-protected. A batch with ANY agent-origin message
// is peer-coalesced; an all-operator batch returns null (operator-rank,
// protected). Callers tag the batch with the highest-privilege member by
// treating any operator presence as protective — i.e. only a purely
// agent-origin batch is downgraded to peer-coalesced.
export function roomBatchTier(messages) {
  const list = Array.isArray(messages) ? messages : [];
  if (list.length === 0) return null;
  return list.every(message => message?.fromAgent === true)
    ? TURN_TIER.PEER_COALESCED
    : null;
}
