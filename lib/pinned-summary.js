...
//
// Two transports. `upsertConvo` is ordinary durable traffic. `upsertConvoRepair`
// is the reconnect fan-out path, which MUST be non-evicting: at hello_ok the
// outbound queue can still hold the whole outage backlog, and a repair frame
// enqueued the ordinary way drops the oldest queued frame — losing real user
// traffic to make room for a digest re-send. Defaults to `upsertConvo` so a
// caller that wires only one transport is unchanged.
//
// The hint is recorded only when the transport did not explicitly refuse: a
// best-effort enqueue that returns false was neither sent nor retained, so
// claiming "already published" there would suppress the very retry that is
// supposed to recover it.
export function summaryJournalPublishEnabled(env = process.env) {
  return (env?.SUMMARY_JOURNAL_PUBLISH ?? '1') !== '0';
}

export function makeJournalSummaryPublisher({ upsertConvo, upsertConvoRepair = upsertConvo, env = process.env } = {}) {
  return function publishJournalSummary(session, summary, { repair = false } = {}) {
    if (!summaryJournalPublishEnabled(env)) return false;
    if (!summary || summary === session._journalSummaryHint) return false;
    const sent = (repair ? upsertConvoRepair : upsertConvo)(session, { summary });
