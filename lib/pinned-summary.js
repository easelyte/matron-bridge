...
// Publishing is a side channel: a throwing publisher must not abort the pass
// and cost the session its title update. Fail LOUD in the log, fail open in
// control flow.
function publishJournalSummary(session, text, publishSummary, warn) {
  try {
  ...
  try {
    ...
    try {
      ...
      if (bulletCount > 15 && currentSummary) {
        if ((session._compactionFailures || 0) < 2) {
          ...
          if (/^•/m.test(compactedSummary)) {
            ...
            publishJournalSummary(session, currentSummary, publishSummary, warn);
      ...
      if (updatedSummary) {
        ...
        publishJournalSummary(session, updatedSummary, publishSummary, warn);
        if (session.claudeSessionId) {
          persistSession(
            session.roomId,
            session.claudeSessionId,
