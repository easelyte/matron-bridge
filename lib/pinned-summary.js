...
// Split a digest into bullet BLOCKS: a line starting with '•' opens a block and
...
//
// Exported because this is the one place that decides what "a bullet" means in
// a stored digest, and BOTH consumers must agree: summaryForJournal drops whole
// blocks so the clamped text never starts with an orphan continuation line, and
// agent-chat's rosterBlurb keeps whole blocks so a bullet's continuation — which
// is where "…but it is blocked on X" lands — is not silently dropped from the
// peer-visible blurb.
...
export function summaryJournalPublishEnabled(env = process.env) {
  return (env?.SUMMARY_JOURNAL_PUBLISH ?? '1') !== '0';
}

// The unchanged-value skip matters beyond bandwidth: a resume backfill or a
// pass that produced no new bullet would otherwise re-send an identical digest,
// and once the server records a summary_updated_at (phase 3) that would stamp
// an old digest as fresh — the exact lie the field exists to prevent.
//
// The digest is DERIVED, low-value, always-recomputable state. It must never
// win a fight against durable traffic, in either direction:
//   - it must not EVICT. The ordinary enqueue drops the OLDEST queued frame
//     when the bounded queue is full, so publishing a digest during an outage
//     could permanently discard a user message or a state transition. The
//     caller therefore wires a non-evicting transport that REFUSES instead.
//   - it must not go STALE IN A QUEUE. A frame held for a later drain is
//     appended at the queue TAIL, so it can land after a newer update of the
//     same field and roll it back. A refusal is therefore reported, never
//     retained, and the caller re-offers whatever the digest is at the moment
//     capacity returns.
//
// Returns 'sent' | 'skipped' | 'refused'. 'skipped' is the kill switch or an
// unchanged digest — nothing to do, nothing to retry. 'refused' means the
// transport took it nowhere, so the hint stays unset and the CALLER must
// schedule a retry: recording "already published" there would suppress the
// very recovery that is supposed to fix it.
export function makeJournalSummaryPublisher({ upsertConvo, env = process.env } = {}) {
  return function publishJournalSummary(session, summary) {
    if (!summaryJournalPublishEnabled(env)) return 'skipped';
    if (!summary || summary === session._journalSummaryHint) return 'skipped';
    if (upsertConvo(session, { summary }) === false) return 'refused';
