...
export function formatBox(box) {
  ...
  try {
    ...
    // `self` is journal-composed (the caller's own device id matched), never peer text.
    const lines = [`${name} (device ${device}) — ${box.online ? 'online' : 'offline'}${box.self === true ? ' — this box' : ''}`];
