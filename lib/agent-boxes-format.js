...
// 1024-based, single-letter units to keep the box line dense. One decimal
// under 10 (7.4G), whole numbers above (233G) — the reader is judging "is
// this box full", not accounting bytes.
function formatBytes(n) {
  const units = ['B', 'K', 'M', 'G', 'T', 'P'];
  let v = n;
  let i = 0;
  while (v >= 1024 && i < units.length - 1) { v /= 1024; i += 1; }
  // The unit is picked before rounding, so a value just under the next
  // boundary rounds back up to it: 1023.9G would render "1024G" rather than
  // "1.0T". Carry it forward when rounding would hit 1024.
  if (v >= 1023.5 && i < units.length - 1) { v /= 1024; i += 1; }
...
export function formatBox(box) {
  ...
  try {
    ...
    // `self` and `wakeable` are journal-composed (the caller's own device id
    // matched; the journal has a wake command for an offline box), never
    // peer text. An asleep box is a valid spawn/invite target — the journal
    // starts it on demand (wake-before-spawn) — so it is named as such rather
    // than lumped in with a box nothing can reach.
    const state = box.online ? 'online' : box.wakeable === true ? 'asleep (woken on demand; allow a few minutes)' : 'offline';
    const lines = [`${name} (device ${device}) — ${state}${box.self === true ? ' — this box' : ''}`];
    ...
    // Numbers, not peer text, so no peerField — but a bridge that predates
    // the journal's sanitizeSpawnDisk could relay garbage through an old
    // journal, so gate on shape here too and skip the line (not the box)
    // when it doesn't hold.
    if (box.disk && Number.isFinite(box.disk.free_bytes) && Number.isFinite(box.disk.total_bytes)
        && box.disk.free_bytes >= 0 && box.disk.total_bytes > 0 && box.disk.free_bytes <= box.disk.total_bytes) {
      const pct = Math.round((box.disk.free_bytes / box.disk.total_bytes) * 100);
      lines.push(`  disk: ${formatBytes(box.disk.free_bytes)} free of ${formatBytes(box.disk.total_bytes)} (${pct}% free)`);
    }
    return lines.join('\n');
  } catch {
