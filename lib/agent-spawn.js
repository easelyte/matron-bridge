...
export function createAgentSpawnHandlers({
    ...
    switch (frame.outcome) {
      case 'started':
        ...
        return frame.room_id
          ? `🚀 Spawn ${id}: ${prefix}session started on the target box. Chat room ${safeFrameField(frame.room_id)} is the channel; the child was seeded with your task and will report there. Child conversation: ${safeFrameField(frame.child_convo_id)}.`
          : `🚀 Spawn ${id}: ${prefix}session started on the target box, detached — it was seeded with your task and will not report back. Child conversation: ${safeFrameField(frame.child_convo_id)}. If you later need a channel to it, agent_chat_start can open one.`;
      case 'declined':
  ...
  return {
    ...
    async sessionStart(data) {
      ...
      if (err) return err;
      const { device_id: deviceId, workdir, task, topic, model, link } = data || {};
