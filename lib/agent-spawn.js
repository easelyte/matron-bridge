...
export function createAgentSpawnHandlers({
  ...
  // A raw ')' in a peer-controlled id would close a `[title](matron://…)`
  // link's parens early and splice arbitrary markdown into a notice the
  // bridge signs and publishes into the user's own chat — encodeURIComponent
  // alone doesn't stop that: '(' and ')' sit in its unreserved set (an
  // ECMA-262 quirk carried from RFC 2396) and pass through raw, so they get
  // an explicit extra pass on top of it.
  //
  // encodeURIComponent itself can throw URIError on a lone UTF-16 surrogate.
  // peerField (used to build safeFrameField's output, the usual input here)
  // does not filter those code units, and its length cap can create one by
  // slicing mid-character — so this is reachable without any hostile intent,
  // just an astral character landing on the truncation boundary. Caller
  // (handleOutcome) tombstones the spawn before this runs, so an uncaught
  // throw here would permanently drop the spawn-started notice rather than
  // just render it oddly — worth a fallback, not just a wider try/catch
  // upstream. On throw, every surrogate code unit (valid pairs included) is
  // replaced with U+FFFD and encoding is retried; that retry cannot throw
  // for the same reason, since no surrogate code units remain.
  function linkSafeId(value) {
    let encoded;
    try {
      encoded = encodeURIComponent(value);
    } catch {
      encoded = encodeURIComponent(value.replace(/[\uD800-\uDFFF]/g, '�'));
    }
    return encoded.replace(/[()]/g, (c) => (c === '(' ? '%28' : '%29'));
  }
  ...
  function describeOutcome(frame, ctx) {
    ...
    switch (frame.outcome) {
      case 'started': {
        // A room_id means the ask was linked (link: true); its absence is
        // the default detached spawn, not a journal omission — say so,
        // rather than name a room that does not exist.
        const childConvoId = safeFrameField(frame.child_convo_id);
        // The "Child conversation: <id>." clause keeps this sanitized-but-
        // unencoded id, unchanged for anything parsing it; only the markdown
        // link target needs the extra linkSafeId encoding (see its own
        // comment above). No real id, no link: safeFrameField's 'unknown'
        // fallback (a missing/empty child_convo_id) can't identify the child
        // conversation, so a matron://convo/unknown link would just be dead
        // weight — the whole clause is built with or without it up front,
        // rather than interpolating an empty hint into a fixed template and
        // leaving a stray double space behind.
        const childConvoClause = childConvoId === 'unknown'
          ? `Child conversation: ${childConvoId}.`
          : `Child conversation: ${childConvoId}. Link it for the user as [title](matron://convo/${linkSafeId(childConvoId)}).`;
        return frame.room_id
          ? `🚀 Spawn ${id}: ${prefix}session started on the target box. Chat room ${safeFrameField(frame.room_id)} is the channel; the child was seeded with your task and will report there. ${childConvoClause}`
          : `🚀 Spawn ${id}: ${prefix}session started on the target box, detached — it was seeded with your task and will not report back. ${childConvoClause} If you later need a channel to it, agent_chat_start can open one.`;
      }
      case 'declined':
  ...
  return {
    ...
    async sessionStart(data) {
      ...
      if (err) return err;
      const { device_id: deviceId, workdir, task, topic, model, link, mission } = data || {};
      ...
      // Optional: open a chat room between this session and the child.
      // Off by default — a spawn is a clean break; the child does its task
      // and does not report back. Only a literal true reaches the wire
      // (the journal treats an absent key as detached), and anything
      // non-boolean is refused here rather than coerced.
      if (link !== undefined && typeof link !== 'boolean') {
        return { status: 400, body: { error: 'link must be a boolean' } };
      }
      ...
      // Optional model for the spawned session. Validated HERE as well as on
      // the target bridge: a rejection the calling agent can read and correct
      // beats a spawn that dies on the far side with a bogus --model.
      const wantsModel = model !== undefined && model !== null && model !== '';
      if (wantsModel && (typeof model !== 'string' || !isValidModelArg(model))) {
        return { status: 400, body: { error: `model must be one of: ${VALID_ALIAS_HINT} (or a full claude-* model name)` } };
      }
      const spawnModel = wantsModel ? normalizeModelArg(model) : null;
      ...
      const frame = {
        ...
        ...(link === true ? { link: true } : {}),
        ...(hasMission ? { mission_num: mission } : {}),
      };
      ...
      if (r.kind === 'op_error') {
        ...
        // The journal only refuses an offline target it cannot wake (no wake
        // command, or the box is unknown to every host); an asleep box it can
        // wake is parked instead and acked with target_waking (below).
        if (r.code === 'agent_unreachable') return { status: 502, body: { error: 'target box is offline and cannot be woken from here; ask the user to start it, then try again' } };
        if (r.code === 'not_found') return { status: 404, body: { error: r.detail || 'target not found' } };
        if (r.code === 'no_mission') return { status: 404, body: { error: `there is no mission #${mission} visible here — the number may be wrong (check with mission_get), or the mission is hidden by privacy (a mission from a private box can only go to a private box: pick a private box, or ask the user); nothing was sent to the user` } };
      ...
      if (r.kind === 'pending' && r.spawnId) {
        ...
        return { status: 200, body: {
          status: 'pending', spawn_id: r.spawnId,
          ...(hasMission ? { mission_num: mission } : {}),
          // Journal-composed flag (wake-before-spawn): the box was asleep and
          // the journal has asked the infra layer to start it. The session
          // begins once the user approves AND the box is up, so the caller's
          // tool copy can set expectations instead of reading a slow start
          // as a failure.
          ...(r.targetWaking ? { target_waking: true, note: 'the target box is asleep and is being woken; the session starts once the user approves and the box is up, which takes a few minutes for a cold start' } : {}),
        } };
      }
    ...
    onSpawnFrame(frame) {
      ...
      if (frame.event === 'pending') {
        settle(frame.request_id, {
          kind: 'pending',
          spawnId: typeof frame.spawn_id === 'string' ? frame.spawn_id : null,
          // Only a literal true counts — an older journal sends no key at all.
          targetWaking: frame.target_waking === true,
        });
        return;
      }
