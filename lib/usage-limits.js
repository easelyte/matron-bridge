...
// resets_at_ms rather than emitting null. The inner ".*?" stays lazy so the
// with-resets case still captures the reset text unchanged.
...
// The session id is plain "session" (the WIRE CONTRACT upstream canonicalised):
// the parsed text says only "session", never "5h", so baking a window guess into
// a "stable" id would be an unverifiable claim clients might persist against.
// Adopted from upstream to end the fork's session_5h / week_all_models wire fork
// (easelyte 2026-09-19, operator-approved): a client built against the other
// side's bridge otherwise mislabels the meters. The weekly descriptor is taken
// from the parenthetical when present, else from the suffix after "week", then
// normalized the SAME way regardless of parens so "all models" always yields
// "week_all" whether or not it was parenthesized (previously the paren-less form
...
// Ensure a machine id is unique within one parsed response. Clients key by id,
// so two entries sharing one (e.g. both falling back to week_other) would
// clobber each other. Disambiguate the second+ collision with a numeric suffix.
...
// { ok, lines } where each line is
//   { id, label, percent, resets, resets_at, resets_at_ms }.
...
//
// resets_at is kept as the original ISO-8601 STRING (its deployed wire type —
// a strict decoder on an older/other client, e.g. iOS Swift Codable, would
// break on a type change under the same key). The epoch number rides a NEW
// sibling key resets_at_ms so clients can drive live countdowns without
// re-parsing a timezone. Both are present when the reset text parses, both
// omitted (never null) when it doesn't.
export function parseUsageLimits(rawText, now = new Date()) {
  ...
  for (const line of String(rawText ?? '').split('\n')) {
    ...
    // "Current week (Fable): 0% used"). Keep resets undefined in that case and
    // omit the resets/resets_at/resets_at_ms keys entirely — same shape as the
    // existing unparseable-resets path (never emit null).
    const resets = m[3] === undefined ? undefined : m[3].trim();
    const entry = {
      ...
      // Deduped so two lines can't share an id and clobber each other.
      id: uniqueId(deriveLimitId(rawLabel), seenIds),
    ...
    // resets_at stays an ISO string (unchanged wire type); resets_at_ms adds
    // the epoch number. All three (resets, resets_at, resets_at_ms) are present
    // only when a resets clause exists AND parses; otherwise all omitted.
    if (resets !== undefined) {
      entry.resets = resets;
      const resetMs = resetsAtMs(resets, now);
      if (resetMs !== null) {
        entry.resets_at = new Date(resetMs).toISOString();
        entry.resets_at_ms = resetMs;
      }
    }
    lines.push(entry);
  }
