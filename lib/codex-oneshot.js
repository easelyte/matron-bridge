import os from 'node:os';
import fs from 'node:fs';
import path from 'node:path';
import { spawn as nodeSpawn } from 'node:child_process';

const KILL_GRACE_MS = 3_000;
const MAX_OUTPUT_BYTES = 256 * 1024;
const MAX_TEXT_CHARS = 4_096;
const MAX_STDERR_TAIL_CHARS = 500;
const DEFAULT_TIMEOUT_MS = 60_000;
const MIN_TIMEOUT_MS = 1_000;
const MAX_TIMEOUT_MS = 300_000;

export function parseTimeout(value) {
  const timeout = Number(value);
  return Number.isInteger(timeout) && timeout >= MIN_TIMEOUT_MS && timeout <= MAX_TIMEOUT_MS
    ? timeout
    : DEFAULT_TIMEOUT_MS;
}

// Run a contained, ephemeral Codex turn for cosmetic summary generation.
// Every failure resolves to a structured result so callers can fail open.
export async function codexOneShot(prompt, {
  model = process.env.SUMMARY_CODEX_MODEL || null,
  timeoutMs = parseTimeout(process.env.SUMMARY_CODEX_TIMEOUT_MS),
  cwd,
  spawnImpl = nodeSpawn,
  command = 'codex',
} = {}) {
  const startedAt = Date.now();
  const args = [
    'exec',
    '--json',
    '--skip-git-repo-check',
    '-s',
    'read-only',
    '-c',
    'approval_policy="never"',
    '--ignore-user-config',
    '--ephemeral',
    ...(model ? ['-m', model] : []),
    '-',
  ];
  const env = {
    PATH: process.env.PATH,
    HOME: process.env.HOME,
    ...(process.env.CODEX_HOME ? { CODEX_HOME: process.env.CODEX_HOME } : {}),
  };

  return new Promise((resolve) => {
    let child;
    let settled = false;
    let timeoutTimer = null;
    let killTimer = null;
    let terminalReason = null;
    let stdout = '';
    let stdoutBytes = 0;
    let stderrBytes = 0;
    let stderrTail = '';
    let privateCwd = null;
    let privateCwdCleanupStarted = false;

    const result = (text, reason, exitCode = null, signal = null) => ({
      text,
      reason,
      exitCode,
      signal,
      durationMs: Date.now() - startedAt,
      stderrTail,
    });

    const cleanupPrivateCwd = () => {
      if (!privateCwd || privateCwdCleanupStarted) return;
      privateCwdCleanupStarted = true;
      fs.rm(privateCwd, { recursive: true, force: true }, () => {});
    };

    const resolveOnce = (value) => {
      if (settled) return;
      settled = true;
      if (timeoutTimer) clearTimeout(timeoutTimer);
      resolve(value);
    };

    const kill = (signal) => {
      if (child?.pid) {
        try {
          process.kill(-child.pid, signal);
          return;
        } catch {
          // Fall back to the direct child if its process group is already gone.
        }
      }
      try {
        child?.kill(signal);
      } catch {
        // Process termination is best-effort; the guarded result still resolves.
      }
    };

    const scheduleForceKill = () => {
      if (settled && terminalReason !== 'output-overflow') return;
      killTimer = setTimeout(() => {
        kill('SIGKILL');
        if (terminalReason === 'timeout') {
          resolveOnce(result(null, 'timeout'));
        }
      }, KILL_GRACE_MS);
    };

    const handleOverflow = () => {
      if (terminalReason) return;
      terminalReason = 'output-overflow';
      kill('SIGTERM');
      scheduleForceKill();
      resolveOnce(result(null, 'output-overflow'));
    };

    // A stdin EPIPE (or a throw from stdin.end) means we can no longer drive the
    // child, but the child may still be alive — resolving without terminating it
    // would leak the process (and its quota) indefinitely. Route through the same
    // SIGTERM -> grace -> SIGKILL termination as overflow before resolving.
    const handleStdinError = () => {
      if (terminalReason) {
        resolveOnce(result(null, 'stdin-error'));
        return;
      }
      terminalReason = 'stdin-error';
      kill('SIGTERM');
      scheduleForceKill();
      resolveOnce(result(null, 'stdin-error'));
    };

    const appendOutput = (stream, chunk) => {
      if (terminalReason) return;
      const bytes = Buffer.isBuffer(chunk) ? chunk.length : Buffer.byteLength(String(chunk));
      if (stream === 'stdout') {
        if (stdoutBytes + bytes > MAX_OUTPUT_BYTES) {
          handleOverflow();
          return;
        }
        stdoutBytes += bytes;
        stdout += chunk.toString();
        return;
      }
      if (stderrBytes + bytes > MAX_OUTPUT_BYTES) {
        handleOverflow();
        return;
      }
      stderrBytes += bytes;
      stderrTail = `${stderrTail}${chunk.toString()}`.slice(-MAX_STDERR_TAIL_CHARS);
    };

    const lastAgentMessage = () => {
      let text = null;
      for (const line of stdout.split('\n')) {
        const trimmed = line.trim();
        if (!trimmed) continue;
        try {
          const event = JSON.parse(trimmed);
          if (
            event.type === 'item.completed'
            && event.item?.type === 'agent_message'
            && typeof event.item.text === 'string'
          ) {
            text = event.item.text.slice(0, MAX_TEXT_CHARS);
          }
        } catch {
          // Ignore non-JSON diagnostic lines; absence of an agent message fails open.
        }
      }
      return text;
    };

    try {
      if (cwd === undefined) {
        privateCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-oneshot-'));
      }
      child = spawnImpl(command, args, {
        cwd: privateCwd || cwd,
        env,
        stdio: ['pipe', 'pipe', 'pipe'],
        detached: true,
      });
    } catch {
      cleanupPrivateCwd();
      resolveOnce(result(null, 'spawn-error'));
      return;
    }

    try {
      child.on('error', () => {
        resolveOnce(result(null, 'spawn-error'));
      });
      child.stdin.on('error', () => {
        handleStdinError();
      });
      child.stdout.on('data', chunk => appendOutput('stdout', chunk));
      child.stderr.on('data', chunk => appendOutput('stderr', chunk));
      child.on('close', (exitCode, signal) => {
        if (killTimer) clearTimeout(killTimer);
        cleanupPrivateCwd();
        if (terminalReason) {
          resolveOnce(result(null, terminalReason, exitCode, signal));
          return;
        }
        if (exitCode !== 0) {
          resolveOnce(result(null, 'nonzero-exit', exitCode, signal));
          return;
        }
        const text = lastAgentMessage();
        resolveOnce(text === null
          ? result(null, 'no-output', exitCode, signal)
          : result(text, null, exitCode, signal));
      });

      timeoutTimer = setTimeout(() => {
        if (settled) return;
        terminalReason = 'timeout';
        kill('SIGTERM');
        if (!settled) scheduleForceKill();
      }, timeoutMs);

      child.stdin.end(prompt);
    } catch {
      handleStdinError();
    }
  });
}
