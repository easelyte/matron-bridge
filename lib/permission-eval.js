...
function readPermissions(sourcePath) {
  try {
    ...
    const ruleLists = [permissions.allow, permissions.deny, permissions.ask];
    const uncertain = ruleLists.some(rules => (
      rules !== undefined
      && (!Array.isArray(rules) || rules.some(rule => typeof rule !== 'string'))
    ));
    return { permissions, uncertain };
  } catch (_error) {
    return { permissions: null, uncertain: true };
  }
}
...
export function classifyPermission(snapshot, toolName) {
  ...
  if (!snapshot.uncertain && serverRules.length > 0 && hasRule(snapshot.mcpAllow, toolName)) {
    return 'allow';
  }
