...
export function canSwitchAgent(session, targetAgent, { hasInflightMedia = false } = {}) {
  ...
  if (session.busy || session._awaitingInputReady || session.queuedMessages?.length || session._codexSteerPending) {
    return {
      ok: false,
  ...
  if (
    ...
    session._codexAwaitingAnswer ||
    session.codexPrompts?.active ||
  ...
  if (session.pendingPlan || session.pendingPlanDenialId || session.ivPendingPlanToolUseId || session._codexBuildValue) {
    return {
      ok: false,
...
function normalizeStoredAgentState(state = {}) {
  ...
  return {
    ...
    ...(state.effort !== undefined ? { effort: typeof state.effort === 'string' ? state.effort : null } : {}),
...
export function snapshotAgentState(session, historyCursor = session?.chatHistory?.length || 0) {
  ...
  return {
    ...
    model: session?.agent === AGENT_CODEX ? session.codex?.model || null : session?.currentModel || null,
    ...(session?.agent === AGENT_CODEX ? { effort: session.codex?.effort || null } : {}),
