...
export async function shareAgentMedia({
  ...
  // tab. realPath already cleared validateAndOpen's fd-pinned containment, so we gate the link on
  // the pinned READ-ROOT (roots[0].realPath) — that re-runs the sensitive-name check and keeps the
  // link within the same boundary the bytes came from. Null (WEB_BASE_URL unset, or a sensitive
  // name) → caption is left exactly as the agent passed it (plain-path fallback). Folded in BEFORE
  // the dedup key so a re-show with the same content+caption still dedups (the link is deterministic
  // from realPath, so it does not perturb the identity).
  const rootDir = Array.isArray(pinnedRoots?.roots) && pinnedRoots.roots.length ? pinnedRoots.roots[0].realPath : null;
