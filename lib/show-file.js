...
export function denialToStatus(reason) {
  if (reason === 'sensitive' || reason === 'outside-scope' || reason === 'path-race') return 403;
...
export async function shareAgentMedia({
  ...
  // Files deep link for the operator (loop #739): open the shown file in the browsable web Files
  // tab. realPath already cleared validateAndOpen's fd-pinned containment against the FULL pinned
  // root set (cwd + every SHOW_FILE_ARTIFACT_ROOTS), so we gate the link on whichever pinned root
  // actually CONTAINS realPath — not just roots[0], which would silently drop the link for any file
  // under a secondary artifact root. That re-runs the sensitive-name check and keeps the link within
  // the same boundary the bytes came from. Null (WEB_BASE_URL unset, or a sensitive name) → caption
  // is left exactly as the agent passed it (plain-path fallback). Folded in BEFORE the dedup key so
  // a re-show with the same content+caption still dedups (the link is deterministic from realPath).
  const rootDir = Array.isArray(pinnedRoots?.roots)
    ? (pinnedRoots.roots.find((r) => r?.realPath && (realPath === r.realPath || realPath.startsWith(`${r.realPath}/`)))?.realPath ?? null)
    : null;
  const deepLink = buildFilesDeepLink(realPath, rootDir, deps.webBaseUrl);
  const effectiveCaption = appendFilesDeepLink(caption, deepLink, filename) || undefined;
  ...
  //
  // Dedup identity keys on a LOCAL digest of the already-buffered bytes, NOT the
  // journal-echoed media.sha256. Keying on the external digest is fragile: a
  // stale/constant/format-drifted server value could suppress changed content or
  // silently no-op dedup under version skew. A local sha256 is deterministic and
  // self-consistent. We still surface media.sha256 in the result (external audit
  // contract), falling back to the local digest when the /media response omits it.
  ...
  const captionKey = effectiveCaption ? effectiveCaption : undefined;
  let ledgerKey;
  if (deps.dedupLedger) {
    try {
      ledgerKey = dedupKey({ token, realPath, sha256: contentSha, caption: captionKey });
