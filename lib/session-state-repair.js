...
//
// Returns { reoffer: [{convoId, state}], retire: [convoId] }.
export function selectEpochRepairs(entries, liveConvoIds) {
  ...
  for (const entry of entries || []) {
    ...
    if (!live.has(convoId) && state === 'running') retire.push(convoId);
    else if (typeof state === 'string' && state) reoffer.push({ convoId, state });
  }
