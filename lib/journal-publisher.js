...
// Ceiling on a single inbound blob download (connect + body read). A stalled
// connection — headers received, then the path goes dead — leaves the body
// read pending forever with no error and no close, and fetchMedia's callers
// are not merely blocked: the journal media router holds a multi-attachment
// batch open across this call, so an unbounded fetch strands the sibling
// attachments that DID arrive. Generous, because this pulls up to
// fetchMediaMaxBytes (25MB) over whatever link the journal server is on —
// this is a stall ceiling, not a latency budget.
const DEFAULT_FETCH_MEDIA_TIMEOUT_MS = 60_000;
...
export function peerMessageIdemKey(fromConvo, targetConvo, body, priority = false) {
  // priority is part of the semantic identity: a normal message and a same-body priority
  // ESCALATION are distinct intents, so they must get distinct keys — otherwise the journal's
  // idempotent append would dedupe the escalation within the 120s window and silently drop it.
  //
  // A normal message KEEPS the legacy 3-element tuple (no trailing discriminator) so its key is
  // byte-identical to the pre-priority producer. Only priority appends the discriminator. Without
  // this, a normal-message retry that straddles the bridge upgrade (old 3-tuple vs new 4-tuple)
  // would hash differently, defeat the journal's exact-key dedup, and inject a duplicate turn.
  const tuple = priority === true ? [fromConvo, targetConvo, body, true] : [fromConvo, targetConvo, body];
...
// Exported for direct unit coverage, and for index.js's items client, which
// talks to the same host over plain HTTP; the publisher's own uploadMedia
// does the derivation internally.
...
export function createJournalPublisher({
  ...
  // Abort deadline for fetchMedia (see DEFAULT_FETCH_MEDIA_TIMEOUT_MS).
  // Factory-injectable so tests can exercise the stalled-download path
  // without waiting a minute; index.js never overrides the 60s default.
  fetchMediaTimeoutMs = DEFAULT_FETCH_MEDIA_TIMEOUT_MS,
  ...
  // Direct room-op writes (sendRoomOp) bypass `queue`, but flush() still has
  // to wait for the socket to confirm them: the box_status that
  // gracefulShutdown sends is the report every client sees while this box is
  // asleep, and process.exit(0) right behind a bare ws.send() would race the
  // write (permessage-deflate compresses off-thread before anything reaches
  // the socket). `inflightRoomOps` is the count not yet confirmed;
  // `lostRoomOps` counts confirmations that came back as errors (or writes
  // written off when the socket died), so a flush can report drained:false
  // honestly instead of waiting out its timeout.
  let inflightRoomOps = 0;
  let lostRoomOps = 0;
  ...
  function enqueue(frame, onLocalSendComplete, onEvicted) {
    const entry = { frame, sending: false, onLocalSendComplete, onEvicted, evicted: false };
    queue.push(entry);
  ...
  //
  // `retain: false` opts OUT of the pending-repair hold: refuse instead of
  // keeping the frame for a later drain. For DERIVED state (the rolling summary
  // digest) retention is actively wrong — drainPendingRepairs appends at the
  // queue TAIL, so a held snapshot can land AFTER a newer ordinary update of
  // the same field and roll it back. Such a caller recomputes from live state
  // and re-offers on the next capacity/reconnect tick instead.
  function enqueueBestEffort(frame, onLocalSendComplete, { retain = true } = {}) {
    if (queue.length >= queueLimit || pendingRepairs.size > 0) {
      ...
      pendingRepairs.set(repairKey, { frame, onLocalSendComplete });
      return false;
    }
    queue.push({ frame, sending: false, onLocalSendComplete });
    pump();
    return true;
  }
    ...
    for (const entry of queue.slice()) {
      ...
      try {
        socket.send(data, (err) => {
          ...
          if (queue.length === 0) settleFlushWaiters();
  ...
  function markDown(socket) {
    ...
    // Unconfirmed direct writes die with the socket (room ops are never
    // retried): write them off so a waiting flush() settles now, not at its
    // timeout.
    if (inflightRoomOps > 0) {
      lostRoomOps += inflightRoomOps;
      inflightRoomOps = 0;
      settleFlushWaiters();
    }
    scheduleReconnect();
  ...
  // Re-run every registered flush() waiter; each one re-checks the full
  // drain condition (empty queue AND no in-flight room ops) itself.
  function settleFlushWaiters() {
    if (flushWaiters.size === 0) return;
  ...
  // A text frame's body must be a string. Anything else (an object handed
  // into a notice slot by a mis-wired caller, loop #787) would render as raw
  // JSON in every client, so the sink refuses it loudly instead.
  function hasStringTextBody(payload, method) {
    if (typeof payload?.body === 'string') return true;
    warn(`[journal-publisher] ${method} refused a non-string text body (${payload?.body === null ? 'null' : typeof payload?.body})`);
    return false;
  }

  function safePublish(convoId, type, payload, options) {
    try {
      const { idemKey, onLocalSendComplete, onEvicted } = options || {};
      return enqueue({
        op: 'publish',
        convo_id: convoId,
        type,
        ...
        typeof onLocalSendComplete === 'function' ? onLocalSendComplete : undefined,
  ...
  // upload can't OOM the bridge. It is also capped in TIME at
  // fetchMediaTimeoutMs: a connection that stalls mid-body would otherwise
  // leave the await pending forever (and, via the media router's batch hold,
  // strand the attachments that did arrive) — the abort turns that into the
  // ordinary null-plus-warn failure every caller already handles.
  async function fetchMedia(blobRef) {
    ...
    const controller = new AbortController();
    const timeout = setTimeout(() => { try { controller.abort(); } catch { /* best effort */ } }, fetchMediaTimeoutMs);
  ...
  return {
    ...
    upsertConvo(convoId, opts, options) {
      ...
      try {
        ...
        // Optional onLocalSendComplete fires from the ws.send(data, cb) write
        // callback (see pump()) — i.e. once the frame has LEFT THIS PROCESS'S
        // local socket buffer, NOT once the server has committed it. There is
        // no application-level per-frame server ack on this WS publish path, so
        // this is a local-send signal, not a durability/commit ack: a frame can
        // fire this and still never reach the server if the connection drops
        // right after the write. A caller may use it as a best-effort "the send
        // left the wire" hook, but a DURABLE write-ahead record must NOT treat
        // it as proof the state landed — the reconnect reconciliation sweep
        // (republishSessionStates / reconcileStrandedSubagents) is the settle
        // authority for that. Absent for every legacy caller.
        const onLocalSendComplete = typeof options?.onLocalSendComplete === 'function' ? options.onLocalSendComplete : undefined;
        enqueue(frame, onLocalSendComplete);
        return true;
      } catch (e) {
    ...
    upsertConvoBestEffort(convoId, opts, options) {
      try {
        ...
        if (sessionState !== undefined) frame.session_state = sessionState;
        if (parentConvoId !== undefined) frame.parent_convo_id = parentConvoId;
        // Same omit-don't-null discipline as upsertConvo. Carried here so the
        // summary reconnect repair (loop #554 F3) can use the NON-EVICTING path
        // instead of pushing the outage backlog out of a full queue.
        if (summary !== undefined) frame.summary = summary;
        if (sessionOutcome !== undefined) frame.session_outcome = sessionOutcome;
        if (agentKind !== undefined) frame.agent_kind = agentKind;
        // onLocalSendComplete rides the pending-repair record too, so it fires whether
        // the frame goes straight onto the live queue or is retained for a
        // later drain. It fires on LOCAL send completion (ws.send callback), not
        // on a server commit ack (none exists on this path) — a capacity-rejected
        // send never fires it and leaves the frame for retry. Callers that clear a
        // durable record on it are re-offered by the reconnect reconciliation
        // sweep, which is the true settle authority.
        const onLocalSendComplete = typeof options?.onLocalSendComplete === 'function' ? options.onLocalSendComplete : undefined;
        return enqueueBestEffort(frame, onLocalSendComplete, { retain: options?.retain !== false });
      } catch (e) {
    ...
    publishText(convoId, payload, options) {
      if (!hasStringTextBody(payload, 'publishText')) return false;
      return safePublish(convoId, 'text', payload, options);
    },
    publishTextBestEffort(convoId, payload, options) {
      if (!hasStringTextBody(payload, 'publishTextBestEffort')) return false;
      try {
        const { idemKey, onLocalSendComplete } = options || {};
    ...
    publishPromptReply(convoId, payload, options) {
      // options ({idemKey, onLocalSendComplete, onEvicted}) threads a deterministic
    ...
    // Bounded shutdown settle: resolves when the outbound queue drains AND
    // every direct room-op write has been confirmed by the socket, or
    // timeoutMs elapses (spec §4). A dead socket cannot hang shutdown.
    // drained:false also when a room-op write failed while we waited — it is
    // gone (never retried), so there is nothing left to wait for.
    flush({ timeoutMs = FLUSH_TIMEOUT_MS } = {}) {
      if (queue.length === 0 && inflightRoomOps === 0) return Promise.resolve({ drained: true });
      const lostBefore = lostRoomOps;
      return new Promise((resolve) => {
        ...
        const check = () => {
          if (queue.length === 0 && inflightRoomOps === 0) finish(lostRoomOps === lostBefore);
        };
    ...
    sendPeerMessage({ targetConvo, fromConvo, body, priority } = {}) {
      try {
        const frame = {
          ...
          body,
          // priority is part of the idem identity so a same-body priority escalation is NOT
          // deduped against a prior normal send within the 120s window.
          idem_key: peerMessageIdemKey(fromConvo, targetConvo, body, priority === true),
          // Carry the payload key only when true — the journal reconstructs the stored payload
          // field-by-field and includes priority only when present, so a normal peer message
          // keeps its 4-key payload (the byte-identical cross-repo fixture stays unchanged).
    ...
    // Connected-or-refuse, same stance as the ephemerals. Unlike them the
    // write IS confirmation-tracked (see inflightRoomOps): flush() waits for
    // it, so a box_status sent first thing in gracefulShutdown is on the
    // wire before process.exit(0). `true` still means "handed to the socket",
    // not "delivered".
    sendRoomOp(frame) {
      try {
        if (closed || !connected || !ws || ws.readyState !== WebSocket.OPEN) return false;
        ...
        try {
          socket.send(data, (err) => {
            // A callback arriving after markDown replaced/nulled this socket
            // was already written off there; counting it again would
            // double-book the loss against a later flush().
            if (socket !== ws) return;
            if (inflightRoomOps > 0) inflightRoomOps -= 1;
            if (err) lostRoomOps += 1;
            settleFlushWaiters();
          });
        } catch (e) {
          if (inflightRoomOps > 0) inflightRoomOps -= 1;
