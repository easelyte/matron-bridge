...
export function createJournalPublisher({
  ...
  // Reconnect-only repairs are idempotent and will be attempted again on the
  // next connection epoch. Unlike normal durable traffic, they must never
  // evict an older queued frame when the bounded queue is already full.
  function enqueueBestEffort(frame) {
  ...
  function safePublish(convoId, type, payload, options) {
    try {
      ...
      // This acknowledges acceptance into the local durable queue only. A
      // later socket/server rejection or queue eviction is repaired by the
      // reconnect re-emission path and server-side idempotent coalescing.
      return true;
    } catch (e) {
  ...
  return {
    upsertConvo(convoId, opts) {
      ...
      try {
        const { title, sessionState, parentConvoId, sessionOutcome } = opts || {};
    ...
    // backlog wins; the caller skips this idempotent repair until a later
    // reconnect instead of letting the normal overflow policy evict traffic.
    upsertConvoBestEffort(convoId, opts) {
      try {
        ...
        if (parentConvoId !== undefined) frame.parent_convo_id = parentConvoId;
        if (sessionOutcome !== undefined) frame.session_outcome = sessionOutcome;
        return enqueueBestEffort(frame);
      } catch (e) {
    ...
    publishPromptReply(convoId, payload) {
      safePublish(convoId, 'prompt_reply', payload);
    },
