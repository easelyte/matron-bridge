...
export function createJournalPublisher({
  ...
  function enqueue(frame, onDelivered) {
    const entry = { frame, sending: false, onDelivered, evicted: false };
    queue.push(entry);
    if (queue.length > queueLimit) {
      const dropped = queue.shift();
      if (dropped) dropped.evicted = true;
      if (!overflowActive) {
        overflowActive = true;
  ...
  return {
    ...
    publishPermissionRequest(convoId, payload) {
      return safePublish(convoId, 'permission_request', payload);
    },
