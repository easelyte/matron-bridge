...
function noopPublisher() {
  return {
    ...
    fetchConvoMessages() { return null; },
    flushCursor() {},
    close() {},
  };
}
...
export function createJournalPublisher({
  ...
  // Read canonical journal events for a convo (GET /convo/:id/messages, Bearer
  // <agent token>). The agent token authorizes the operator's own convos
  // server-side (authorize() by owner_user_id), so no server change is needed.
  // Returns the parsed `events` array (each {seq, convo_id, ts, sender, type,
  // payload}, oldest-first) or null on any failure — used by index.js's #492
  // durable-picker-provenance seam to resolve a picker frame from the journal
  // when the in-memory shadow is gone (post-restart). Same fail-open contract
  // as fetchMedia: no base URL / bad convoId / network error / non-2xx / bad
  // JSON all resolve to null (plus one warn), never throw, so a lookup failure
  // degrades to ordinary prompt-answer routing rather than crashing the input
  // consumer.
  async function fetchConvoMessages(convoId, { beforeSeq = null, limit = 50 } = {}) {
    if (!mediaHttpBaseUrl) return null;
    if (typeof convoId !== 'string' || convoId.length === 0) {
      warn('[journal-publisher] fetchConvoMessages called with no convo_id');
      return null;
    }
    try {
      ...
      const res = await fetch(`${mediaHttpBaseUrl}/convo/${encodeURIComponent(convoId)}/messages?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
        warn(`[journal-publisher] fetchConvoMessages failed: HTTP ${res.status} (convo=${convoId})`);
        return null;
      }
      const data = await res.json();
      return Array.isArray(data?.events) ? data.events : null;
    } catch (e) {
      warn(`[journal-publisher] fetchConvoMessages failed: ${e.message} (convo=${convoId})`);
      return null;
    }
  }
