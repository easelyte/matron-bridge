...
export async function validateAndOpen(filePath, { workdir, allowedRoots, maxBytes = MAX_VIEW_BYTES } = {}) {
  let fd;
  try {
    ...
    // EOF before the snapshot is complete means the file changed underneath
    // us and must not be published as a successful partial artifact.
    const buf = Buffer.alloc(stat.size);
    let offset = 0;
    while (offset < stat.size) {
      const { bytesRead } = await fd.read(buf, offset, stat.size - offset, offset);
      if (bytesRead === 0) throw new FileLinkDenied('unreadable');
      offset += bytesRead;
    }
    const finalStat = await fd.stat();
    if (finalStat.size !== stat.size) throw new FileLinkDenied('unreadable');
    return { content: buf, realPath };
  } finally {
