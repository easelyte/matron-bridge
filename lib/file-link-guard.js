...
// Hash-based deep link into the web client's Files pane: `${webBaseUrl}/#files=<enc>`.
// webBaseUrl is where the web app itself is served (it owns the `#files=` hash router); it
// must NOT be suffixed with `/journal/`, which on our deploy is the journal API proxy and
// answers 401 JSON (found 2026-09-23: every #739 link opened `{"error":"unauthenticated"}`).
// UNLIKE generateFileLink (index.js), which mints a signed, TTL'd viewer token, this carries NO
// token: the target is opened through the operator's EXISTING web session (the same auth every
// other Files read uses), so there is nothing to sign — a bearer-in-URL would only be a leak
// risk. Gated by the SAME lexical checkFileLink used for viewer links (target within the
// session's workdir/read-root AND not a sensitive-looking path), so a doc handoff for an
// out-of-root or sensitive file returns null and the caller falls back to the plain path.
// Returns null when webBaseUrl is unset (feature dormant) or the gate denies.
export function buildFilesDeepLink(filePath, workdir, webBaseUrl) {
  if (!webBaseUrl || !filePath) return null;
  if (!path.isAbsolute(String(filePath))) return null;
  const absTarget = path.resolve(String(filePath));
  const gate = checkFileLink(absTarget, workdir ? path.resolve(String(workdir)) : null);
  if (!gate.ok) return null;
  const base = String(webBaseUrl).replace(/\/+$/, '');
  return `${base}/#files=${encodeURIComponent(absTarget)}`;
}

// Append an operator-visible "open in Files" line to an attachment caption or item body. Additive:
// the caller's own text stays first and the deep link is a trailer, so nothing is ever replaced.
// A null/absent deep link (webBaseUrl unset, or the target failed the gate) returns the text
// verbatim — that is the plain-path fallback. `name`, when given, labels the link with the file.
export function appendFilesDeepLink(text, deepLink, name) {
  if (!deepLink) return typeof text === 'string' ? text : (text ?? '');
  const label = name ? `📁 Open ${name} in Files` : '📁 Open in Files';
  const line = `${label}: ${deepLink}`;
  const base = typeof text === 'string' && text.trim() ? text : '';
  return base ? `${base}\n\n${line}` : line;
}
