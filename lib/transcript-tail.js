...
// already exists. Pass { startOffset: N } to start at a KNOWN byte boundary
// instead of either extreme: a resumed subagent's transcript must resume at the
// size the file had when the prior run ended, because anything appended between
// then and the moment the tail is attached is new output that EOF would silently
// discard (see SubagentWatcher.forceAttach). readFromStart wins if both are set;
// a truncation/replacement below the offset is handled by _tick as usual.
...
export class TranscriptTail extends EventEmitter {
  constructor(filePath, {
    ...
    this.startOffset = Number.isInteger(startOffset) && startOffset >= 0 ? startOffset : null;
    ...
    this.resourceLimitEmitted = false;
    this.offset = 0;
    this.lineBuf = '';
  ...
  async start() {
    ...
    try {
      if (this.readFromStart) {
        ...
        // A caller-supplied boundary. Clamp to the current size: a file that has
        // since been truncated/replaced must not start past its own end (_tick
        // also self-heals that, but don't rely on it for the very first read).
        let size = this.startOffset;
        try { size = Math.min(this.startOffset, fs.statSync(this.filePath).size); }
        catch { /* not on disk yet — _tick re-reads from this offset when it lands */ }
        this.offset = size;
        this.discardUntilNewline = false;
  ...
  async drain(options = {}) {
    try {
      ...
      // Shared callers may deliberately fire-and-forget drain(). Never expose
      // a rejected promise; strict consumers inspect this explicit result.
      return { ok: false, error };
    }
  }

  _tick({ throwOnFileError = false } = {}) {
    if (this.resourceLimitEmitted) return false;
    let fd;
    try {
    ...
    try {
      ...
      if (Number.isFinite(this.maxFileSizeBytes) && unreadBytes > this.maxFileSizeBytes) {
        ...
        const nextOffset = this.offset + skippedBytes;
        const boundary = Buffer.allocUnsafe(1);
        const boundaryRead = nextOffset > 0
          ? fs.readSync(fd, boundary, 0, 1, nextOffset - 1)
          : 0;
        this.offset = nextOffset;
        this.lineBuf = '';
        this.decoder = new StringDecoder('utf8');
        this.discardUntilNewline = boundaryRead === 1 && boundary[0] !== 0x0a;
      ...
      while (this.offset < stat.size) {
        if (this.resourceLimitEmitted) break;
        const remainingBudget = this.maxTotalBytes - this.totalBytesRead;
        if (Number.isFinite(this.maxTotalBytes) && remainingBudget <= 0) {
          this._emitResourceLimit('bytes');
          break;
        }
        const len = Math.min(DEFAULT_READ_CHUNK_BYTES, stat.size - this.offset, remainingBudget);
  ...
  _emitResourceLimit(resource) {
    if (this.resourceLimitEmitted) return;
    this.resourceLimitEmitted = true;
    if (this.timer) clearInterval(this.timer);
    this.timer = null;
    this.started = false;
    const error = new Error(`transcript cumulative ${resource} limit exceeded`);
    error.code = 'TRANSCRIPT_RESOURCE_LIMIT';
    error.resource = resource;
    this.emit('resourceLimit', error);
  }

  terminateForLimit(resource) {
    this._emitResourceLimit(resource);
  }
