...
import { isPickerValue } from './picker-dispatch.js';
...
export function createJournalInputConsumer({
  ...
  function onJournalEvent(frame) {
    try {
      ...
      // Picker taps (/model, /effort, /mode) — same situation as queue
      // actions above: their `choice` carries a bridge-controlled namespaced
      // VALUE (`model:<alias>` / `effort:<level>` / `mode:<target>`, see
      // lib/picker-dispatch.js), the picker frame never advances the
      // staleness guard (non-answerable, issue #98), and they answer no
      // pending prompt. Route them around the guard so a tap after any
      // answerable prompt isn't wrongly refused as stale; the receiving
      // routePromptReply (index.js journalOnPromptReply) classifies with the
      // same predicate and applies the switch (loop #461).
      if (isPickerValue(payload?.choice)) {
        routePromptReply(session, {
          target_seq: payload?.target_seq ?? null,
          choice: payload.choice,
          text: payload?.text ?? null,
