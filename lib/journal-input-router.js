...
import { buildPermissionKey } from './permission-registry.js';
...
//   notePermissionSeq(key, seq, convoId) -> void (consumed by T-2.3)
    ...
    try {
      ...
      if (type === 'prompt' && Number.isInteger(frame.seq)) {
        ...
        if (typeof sender === 'string' && sender.startsWith('agent:')
          ...
          const alreadyRegistered = frames?.has(frame.seq)
            || (frames && [...frames.values()].includes(key));
          if (!alreadyRegistered && typeof isLivePendingToolUse === 'function'
            ...
            if (typeof notePermissionSeq === 'function') {
