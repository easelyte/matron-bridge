...
//   publishPromptReply(convoId, {target_seq, choice, text}) -> void (optional decision-label echo)
...
export function createJournalInputConsumer({
  ...
  function resolvePermissionDecision(convoId, targetSeq, key, decision) {
    if (typeof resolvePermissionReply !== 'function') return;
    resolvePermissionReply(key, decision);
    if (typeof publishPromptReply !== 'function') return;
    const choice = decision === 'allow' ? 'Allow' : 'Deny';
    try {
      publishPromptReply(convoId, {
        target_seq: targetSeq,
        choice,
        text: null,
      });
    } catch (e) {
      warn(`[journal-input] permission decision echo failed: ${e.message}`);
    }
  }
  ...
  function drainPermissionReplyBuffer(convoId, seq) {
    ...
    const decision = typeof rawDecision === 'string'
      ...
      ? 'allow'
      : 'deny';
    resolvePermissionDecision(convoId, seq, key, decision);
  }
  ...
  function onJournalEvent(frame) {
    try {
      ...
      if (resolvedPermissionSeqsByConvo.get(convoId)?.has(targetSeq)) return;

      const permissionReplyIsCausal = Number.isInteger(frame.seq)
        && Number.isInteger(targetSeq)
        && targetSeq < frame.seq;
      let permissionReplyRejected = false;
      ...
      if (targetSeq != null && permFrames?.has(targetSeq)) {
        if (permissionReplyIsCausal) {
          ...
          const decision = typeof payload?.choice === 'string'
            ...
            ? 'allow'
            : 'deny';
          resolvePermissionDecision(convoId, targetSeq, key, decision);
          return;
        }
        permissionReplyRejected = true;
      }
