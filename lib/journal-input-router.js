...
// oldest are dropped past this window. The same bound applies to queue-action
// frames.
const PICKER_FRAME_RETENTION = 16;
const QUEUE_ACTION_FRAME_RETENTION = 16;
...
export function createJournalInputConsumer({
  ...
  // Queue-action frames ("📨 Queued" tiles) the bridge published per convo, as
  // a per-convo Set of seqs. A prompt_reply is routed as a queue control ONLY
  // when its target_seq names one of these frames (explicit provenance, #493),
  // with the value-shape check kept as a fallback confirmation — never as the
  // sole classifier. This is what stops an ordinary AskUserQuestion whose
  // option value happens to be `interrupt`/`cancel:<n>` from being swallowed as
  // a queue control: that reply targets an ANSWERABLE frame, not a queue-action
  // frame, so it stays an ordinary answer (delivered, visible). Same in-memory
  // / evict-on-teardown / bounded contract as pickerFramesByConvo; not
  // single-use (a queue tile can be tapped more than once — a second tap is a
  // handled no-op, so keeping the frame recorded keeps retries routed as queue
  // controls rather than falling through to a stale refusal).
  const queueActionFramesByConvo = new Map();
    ...
    try {
      ...
      if (type === 'prompt' && Number.isInteger(frame.seq)) {
        if (promptExpectsReply(payload)) {
          ...
          // Record the seq of this queue-action tile so a later reply targeting
          // it is recognized as a queue control by provenance, not value shape.
          let seqs = queueActionFramesByConvo.get(convoId);
          if (!seqs) { seqs = new Set(); queueActionFramesByConvo.set(convoId, seqs); }
          seqs.add(frame.seq);
          // Bound growth (Set preserves insertion order): drop the oldest.
          while (seqs.size > QUEUE_ACTION_FRAME_RETENTION) {
      ...
      // value shape alone) is what stops an ordinary AskUserQuestion whose
      // option value happens to be `interrupt`/`cancel:<n>` from being
      // swallowed here: that reply targets an answerable frame, so it falls
      // through to ordinary answer handling (delivered, visible). The receiving
      // routePromptReply (index.js journalOnPromptReply) trusts the explicit
      // `queue_action` flag and runs the shared handler. A reply that carries a
      // queue value but names NO published queue-action frame (legacy tile, or
      // a bridge restart that lost the record) fails safe toward delivery too —
      // it falls through as an ordinary answer, never silently queue-routed.
      const queueFrames = queueActionFramesByConvo.get(convoId);
      const targetsQueueFrame = payload?.target_seq != null
        && queueFrames && queueFrames.has(payload.target_seq);
      if (targetsQueueFrame && isQueueActionValue(payload?.choice)) {
  ...
  onJournalEvent.evictConvo = function evictConvo(convoId) {
    ...
    queueActionFramesByConvo.delete(convoId);
  };
