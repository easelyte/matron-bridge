...
// A picker option VALUE shape (`model:<x>` / `effort:<x>` / `mode:<x>`). Used
// ONLY as a cheap pre-filter for the canonical-journal fallback below (#492):
// an ordinary answer's choice never matches it, so the common reply path stays
// synchronous and issues no journal read. It does NOT authorize a switch on its
// own — the fallback still validates the choice against the canonical picker
// frame's own offered values (isPickerFrame + pickerFrameValues), preserving
// the frame-binding provenance property (loop #461).
const PICKER_VALUE_PREFIX = /^(?:model|effort|mode):/;

// Sentinel stored in a convo's picker-frame map once a frame has been
// dispatched, replacing the prior delete-on-consume. A retained marker (rather
// than a missing entry) is what makes the single-use guard survive the arrival
// of the #492 canonical fallback: a duplicate reply for a consumed seq now
// reads CONSUMED and stops, instead of reading "absent" and re-resolving the
// same frame from the journal to fire the switch a second time.
const CONSUMED = Symbol('picker-frame-consumed');
...
//   resolveTargetPromptPayload(convoId, targetSeq) -> Promise<payload|null> (optional):
//     the canonical journal `prompt` event's payload at targetSeq, or null. The
//     #492 durability seam — the ONLY I/O the picker path performs, and only on
//     an in-memory-record MISS for a picker-shaped reply (post-restart). Kept
//     injectable so this module stays pure/unit-testable; index.js wires it to
//     a journal GET /convo/:id/messages read.
  ...
  function onJournalEvent(frame) {
    try {
      ...
      const offeredValues = (payload?.target_seq != null && pickerFrames)
        ...
        : undefined;
      if (offeredValues && offeredValues !== CONSUMED && offeredValues.has(payload?.choice)) {
        ...
        // fresh frame with a new seq. Marked CONSUMED (not deleted) so a
        // duplicate can't slip into the canonical fallback below and re-resolve.
        pickerFrames.set(payload.target_seq, CONSUMED);
        routePromptReply(session, {
      ...
      // Ordinary prompt-answer routing (staleness guard + dispatch), factored
      // out so the async canonical-picker fallback below can fall through to it
      // on any miss/error without duplicating the logic. A target_seq that
      // doesn't reference the latest prompt we published into this convo means
      // the prompt the user answered has been superseded — refuse rather than
      // mis-answer the newer one. No recorded seq (restart, live-only
      // reconnect) or no target_seq -> nothing to check, accept.
      const dispatchOrdinaryReply = () => {
      ...
      // Durable picker provenance (#492): the in-memory frame is process-local,
      // so after a bridge restart a picker card published pre-restart has NO
      // record here (offeredValues === undefined — distinct from a CONSUMED
      // marker, which is a same-process duplicate and must NOT re-resolve). When
      // a seam is wired and this reply is picker-SHAPED, resolve the frame from
      // the canonical journal event before treating it as an ordinary answer.
      // The value-shape check is only a cheap gate to keep ordinary replies off
      // the network; the fallback still binds the dispatch to the frame's own
      // offered values, so a genuine answer that merely looks like a picker
      // value is never hijacked (same provenance property as the fast path).
      if (resolveTargetPromptPayload
          && payload?.target_seq != null
          && offeredValues === undefined
          && typeof payload?.choice === 'string'
          && PICKER_VALUE_PREFIX.test(payload.choice)) {
        const targetSeq = payload.target_seq;
        const choice = payload.choice;
        Promise.resolve()
          .then(() => resolveTargetPromptPayload(convoId, targetSeq))
          .then((framePayload) => {
            if (framePayload
                && isPickerFrame(framePayload)
                && pickerFrameValues(framePayload).has(choice)) {
              // Record the consumption durably-enough for this process so a
              // client retry can't re-resolve the same journal event and fire
              // the switch twice (the journal event can't be deleted).
              let frames = pickerFramesByConvo.get(convoId);
              if (!frames) { frames = new Map(); pickerFramesByConvo.set(convoId, frames); }
              // Re-check under the (now-settled) map: a same-seq tap resolved
              // first would already have marked it CONSUMED.
              if (frames.get(targetSeq) === CONSUMED) return;
              frames.set(targetSeq, CONSUMED);
              while (frames.size > PICKER_FRAME_RETENTION) {
                frames.delete(frames.keys().next().value);
              }
              routePromptReply(session, {
              ...
              dispatchOrdinaryReply();
            }
          })
          .catch((e) => {
            warn(`[journal-input] canonical picker lookup failed for convo=${convoId} seq=${targetSeq}: ${e.message}`);
            dispatchOrdinaryReply();
          });
        return;
      }
