...
import { isQueueActionValue, isQueueActionFrame } from './busy-queue.js';
...
// Bound on RESOLVED queue-action frames retained per convo (for double-tap
// idempotency). Only ever evicts already-resolved seqs — never a live
// (still-actionable) tile — so an old, spent tile is eventually forgotten while
// every currently-tappable tile is retained no matter how many are outstanding.
const QUEUE_ACTION_RESOLVED_RETENTION = 32;
...
export function createJournalInputConsumer({
  ...
  // Queue-action provenance (#493), split across two per-convo Sets of journal
  // seqs so a queue-control reply is recognized by the FRAME it targets, not by
  // the reply's value shape (which mis-routed an ordinary AskUserQuestion whose
  // option value happened to be `interrupt`/`cancel:<n>`).
  //
  //   liveQueueActionSeqs   — published "📨 Queued" tiles that are still
  //       actionable. A reply targeting one is routed as a queue control
  //       (with the value shape kept as a fallback confirmation). NEVER
  //       recency-evicted: every currently-tappable tile is retained no matter
  //       how many messages are queued (blocker 2 — a recency cap here rejected
  //       the 17th+ still-live tile). Bounded instead by lifecycle: a seq
  //       leaves `live` the instant its tile is acted on (below) or its queue
  //       entry is resolved by a flush (resolveQueueActionFrames, called from
  //       index.js flushQueue).
  //
  //   resolvedQueueActionSeqs — tiles already acted on or flushed. A repeat
  //       reply to one of these is a handled no-op — this is what makes the
  //       mutation idempotent so a double-tapped / retried `cancel:<n>` cannot
  //       splice the queue twice and delete the wrong (index-shifted) message
  //       (blocker 1). Safe to recency-evict (a spent tile losing its record
  //       only means a very-late duplicate degrades to ordinary handling, never
  //       data loss), so this Set alone carries the growth bound.
  const liveQueueActionSeqsByConvo = new Map();
  const resolvedQueueActionSeqsByConvo = new Map();

  // Mark a queue-action seq resolved: drop it from `live` and record it in the
  // bounded `resolved` set so repeats are idempotent no-ops.
  function markQueueActionResolved(convoId, seq) {
    const live = liveQueueActionSeqsByConvo.get(convoId);
    if (live) live.delete(seq);
    let resolved = resolvedQueueActionSeqsByConvo.get(convoId);
    if (!resolved) { resolved = new Set(); resolvedQueueActionSeqsByConvo.set(convoId, resolved); }
    // Refresh recency: delete-then-add moves an existing seq to the newest slot.
    resolved.delete(seq);
    resolved.add(seq);
    while (resolved.size > QUEUE_ACTION_RESOLVED_RETENTION) {
  ...
  function onJournalEvent(frame) {
    try {
      ...
      if (type === 'prompt' && Number.isInteger(frame.seq)) {
        ...
        } else if (isQueueActionFrame(payload)) {
          // Record this "📨 Queued" tile as a live, actionable frame. No
          // recency cap — every currently-tappable tile is retained (blocker
          // 2); the set is bounded by lifecycle (markQueueActionResolved on tap
          // or flush) instead.
          let live = liveQueueActionSeqsByConvo.get(convoId);
          if (!live) { live = new Set(); liveQueueActionSeqsByConvo.set(convoId, live); }
          live.add(frame.seq);
        }
      }
      ...
      // type === 'prompt_reply'. Queue-tile actions first (⚡ Send now /
      // ✕ Cancel): they never answer a pending prompt, and their tile never
      // advances the staleness guard (it's non-answerable, above), so the
      // guard comparison below would wrongly refuse them whenever ANY
      // answerable prompt has been recorded for the convo. Route them around
      // the guard — but ONLY when the reply targets a queue-action FRAME the
      // bridge actually published (explicit provenance, #493), with the value
      // shape kept as a fallback confirmation. Binding to the frame (not the
      // value shape alone) stops an ordinary AskUserQuestion whose option value
      // happens to be `interrupt`/`cancel:<n>` from being swallowed here: that
      // reply targets an answerable frame, so it falls through to ordinary
      // answer handling (delivered, visible). A reply carrying a queue value but
      // naming NO published queue-action frame (legacy tile, or a restart that
      // lost the record) also falls through — fail safe toward delivery, never
      // silent queue-routing.
      const targetSeqQA = payload?.target_seq;
      // Idempotency (blocker 1): a repeat reply to an already-resolved tile
      // (double-tap / client retry of `cancel:<n>`) is a handled no-op — never
      // dispatched again, so the queue is never spliced twice and a shifted
      // sibling message is never silently deleted. Resolved tiles are also the
      // post-flush state, so a late tap on a spent tile is swallowed here rather
      // than mis-delivered as an ordinary answer.
      const resolvedQA = resolvedQueueActionSeqsByConvo.get(convoId);
      if (targetSeqQA != null && resolvedQA && resolvedQA.has(targetSeqQA)) {
        warn(`[journal-input] duplicate/late queue-action reply for resolved tile seq=${targetSeqQA} convo=${convoId} — no-op`);
        return;
      }
      const liveQA = liveQueueActionSeqsByConvo.get(convoId);
      const targetsLiveQueueFrame = targetSeqQA != null && liveQA && liveQA.has(targetSeqQA);
      if (targetsLiveQueueFrame && isQueueActionValue(payload?.choice)) {
        // Consume the tile: resolve it BEFORE dispatch so a concurrent retry
        // can't re-enter the mutation (blocker 1). The receiving routePromptReply
        // (index.js journalOnPromptReply) trusts the explicit `queue_action`
        // flag and runs the shared handler.
        markQueueActionResolved(convoId, targetSeqQA);
        routePromptReply(session, {
          target_seq: targetSeqQA,
  ...
  onJournalEvent.evictConvo = function evictConvo(convoId) {
    ...
    liveQueueActionSeqsByConvo.delete(convoId);
    resolvedQueueActionSeqsByConvo.delete(convoId);
  };

  // Queue-entry lifecycle hook (#493 blocker 2): index.js calls this from
  // flushQueue when a convo's whole queue is sent/flushed at once — every
  // outstanding tile's entry is resolved, so mark ALL its live frames resolved
  // (they become idempotent no-ops rather than lingering as "live" forever).
  // This is what ties frame lifecycle to queue-entry lifecycle so the live set
  // stays bounded to currently-actionable tiles without ever recency-evicting a
  // live one. A single-entry cancel does NOT go through here — that tile is
  // resolved individually when its own reply dispatches. Idempotent and
  // tolerant of unknown convo ids.
  onJournalEvent.resolveQueueActionFrames = function resolveQueueActionFrames(convoId) {
    const live = liveQueueActionSeqsByConvo.get(convoId);
    if (!live || live.size === 0) return;
    for (const seq of [...live]) markQueueActionResolved(convoId, seq);
    liveQueueActionSeqsByConvo.delete(convoId);
  };
