...
// as a result (so an untrusted receipt — see ackTrusted — returns false). Keeping the decision here (rather than inline at the call site)
...
//
// ackTrusted (Codex R2 F1): whether an ack for THIS interrupt means anything.
// An interrupt written before the CLI has emitted the turn's `system/init` is
// silently dropped, and the CLI still answers it with an ordinary success
// receipt — the caller cannot tell "interrupted something" from "there was
// nothing to interrupt yet" (open upstream bug, anthropics/claude-agent-sdk-
// typescript#429, reproduced on CLI 2.1.241; our own probe measured a ~6.9s
// first-turn init window, so it is amply reachable). Promoting that receipt
// would push recovery from timeoutMs out to backstopMs for a turn that was
// never interrupted at all — strictly worse than not consuming the ack. When
// ackTrusted is false, ack() is a no-op and the original timeoutMs deadline
// stands, which is exactly the pre-#701 behaviour. Enforced here rather than
// only at the call site so a future caller cannot lose the guard by forgetting
// to check.
export function sendPrintInterrupt({
  ...
  return {
    ...
    get ackTrusted() { return ackTrusted !== false; },
    cancel: () => {
      settled = true;
      clearTimeoutFn(timer);
    },
    ack: () => {
      if (ackTrusted === false) return false;
