...
export function createRecentFolders({
  ...
  // Atomic write (PR #151): this is the SOLE durable copy of the folder
  // history, so a truncating in-place rewrite is unsafe — a kill / power loss
  // / ENOSPC / short write after truncation corrupts the only file, and the
  // constructor's parse-failure recovery then silently starts empty and the
  // next touch overwrites the evidence, losing every remembered folder.
  // lib/atomic-write.js owns the temp-sibling + rename dance (and the full
  // failure story); last-writer-wins on the final rename is acceptable for an
  // append-only convenience cache — a full read-merge-write lock would be
  // disproportionate here. The durable `file` is untouched by a failed save.
  const save = () => {
    try {
      atomicWriteFileSync(file, JSON.stringify(folders, null, 2), { fs });
    } catch (e) {
