...
export class CodexWatcher extends EventEmitter {
  ...
  _reconcileRun(runId) {
    const metaName = `codex-${runId}.meta.json`;
    const meta = this._readMeta(runId, { quarantineInvalid: true });
    if (!meta) return false;
    let discovery;
    try {
      discovery = this._discover(meta);
    } catch {
      // Restart reconciliation is the final opportunity to recover an old
      // run. A discovery that throws is run-local corruption: retrying it in
      // snapshot and every poll would count against the session breaker and
      // eventually disable healthy siblings.
      this.permanentlySkipped.add(metaName);
      this.reconcilePending.delete(runId);
      return false;
    }
    if (!this.pending.has(runId)) {
      if (discovery === null) this.reconcilePending.add(runId);
      return false;
    }
