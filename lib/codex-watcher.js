...
export class CodexWatcher extends EventEmitter {
      ...
      while (!exhausted) {
        ...
        for (const name of entries) {
          ...
          if (this.reconcilePending.has(runId)) {
            this._guardReconcile(runId, 'reconcile-run', () => this._reconcileRun(runId));
            continue;
          }
    ...
    try {
      ...
      while (!exhausted) {
        for (let count = 0; count < this.maxMetaScanCount; count += 1) {
          ...
          if (this._guardReconcile(runId, 'reconcile-run', () => this._reconcileRun(runId)) !== true) {
            complete = false;
          }
        }
  ...
  _reconcileRun(runId) {
    ...
    if (this.onReconcile(runId, 'interrupted') !== true) {
  ...
  _complete(runId, outcome, winningSignal, livenessEvidence, drainTimedOut = false) {
    ...
    const accepted = this.isolation?.requestTerminalization(runId, completion.outcome, {
      ...
      livenessEvidence,
      finalPostLanded: !drainTimedOut && streamState.finalPostLanded === true,
