...
export function createCodexWatcherIsolation({
  ...
  // journal outage onDelivered never fires, so breakerNoted stays open — without
  ...
  function postBreakerNote() {
    ...
    try {
      ...
      // reconnect replays it. Latch delivery via onDelivered; latch the enqueue
      ...
      if (typeof publisher?.publishTextBestEffort === 'function') {
        ...
        publisher.publishTextBestEffort(parentConvoId, payload, {
          idemKey,
          onDelivered: markBreakerNoteDelivered,
        });
        return;
      }
  ...
  return {
    ...
    pendingBreakerNote() {
      ...
      // replay is itself lost (still no onDelivered) and the breaker keeps
