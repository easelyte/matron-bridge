...
export class CodexWatcher extends EventEmitter {
  constructor({
    ...
    this._guardSession('snapshot', () => this.snapshot());
    this._guardSession('watch', () => this._ensureFsWatch());
    this._guardSession('scan', () => this._scan());
  ...
  attach(runId) {
    ...
    return this._guardRun(runId, 'attach', () => {
      const tail = new this.TailClass(filePath, { readFromStart: true });
      tail.on('event', event => {
  ...
  _scan() {
    ...
    for (const runId of [...this.pending.keys()]) this.attach(runId);
  }
