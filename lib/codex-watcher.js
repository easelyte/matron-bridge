...
export function registerCodexWatcherForSession(session, options, dependencies = {}) {
  return startCodexWatcherIfEnabled(options, {
    ...dependencies,
...
export class CodexWatcher extends EventEmitter {
  ...
  _reconcileRun(runId) {
    const meta = this._readMeta(runId);
    if (!meta) return false;
    const discovery = this._discover(meta);
    ...
    const outcome = hasExitCode(meta)
      ...
      : 'interrupted';
    this.isolation?.requestTerminalization(runId, outcome, {
      winningSignal: hasExitCode(meta) ? 'clean-exit' : 'restart',
      livenessEvidence: null,
      finalPostLanded: false,
    });
