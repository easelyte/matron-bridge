  ...
  function auditBreakerTerminalizations() {
    try {
      ...
      for (const terminalization of terminalizations) {
        ...
        const pendingTerminalization = pendingTerminalizations.get(runId);
        if (pendingTerminalization) pendingTerminalization.terminalized = true;
        auditTerminal(runId, {
...
export class CodexWatcher extends EventEmitter {
        ...
        tail.on('event', event => {
          this._guardRun(runId, 'tail-event', () => {
            ...
            if (event?.type === 'item.completed' && event.item?.type === 'agent_message') {
              streamState.finalMessageSeen = true;
            }
            if (event?.type === 'turn.completed') {
              streamState.terminalItemSeen = true;
              streamState.finalPostLanded = streamState.finalMessageSeen === true;
            }
  ...
  _watchdogTick() {
    ...
    for (const runId of runIds) {
      ...
      const previousMeta = this.pending.get(runId);
      const meta = this._readMeta(runId) ?? previousMeta;
      if (!meta) continue;
      if (hasExitCode(meta)) {
  ...
  _beginDrain(runId, outcome) {
    ...
    const completion = { outcome, winningSignal: 'clean-exit', timer: null };
    this.completions.set(runId, completion);
    const finish = () => {
      completion.timer = null;
      this._guardRun(runId, 'drain', () => this._finishDrain(runId));
    };
    completion.timer = setTimeout(finish, this.drainWindowMs);
  ...
  _finishDrain(runId) {
    ...
    try { this.tails.get(runId)?.drain?.(); } catch { /* isolation owns publication failures */ }
    this._complete(
      runId,
      completion.outcome,
      completion.winningSignal,
      { pidAlive: false, pastDeadline: false },
    );
  }
  ...
  _complete(runId, outcome, winningSignal, livenessEvidence) {
    ...
    const accepted = this.isolation?.requestTerminalization(runId, completion.outcome, {
      ...
      livenessEvidence,
      finalPostLanded: streamState.finalPostLanded === true,
    }) === true;
    try { this.tails.get(runId)?.drain?.(); } catch { /* best-effort final read */ }
