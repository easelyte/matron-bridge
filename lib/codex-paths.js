...
// real codex behind it forwards to nothing. MATRON_CODEX_REAL_BIN counts when
// it resolves, mirroring detectProducer. Feeds the New Chat agent switch: a
// box that answers false is offered as Claude-only rather than with a Codex
// option that can only ever spawn ENOENT.
export function detectCodexBinary({ env = process.env, shimPath = SHIPPED_SHIM_PATH, realpath = safeRealpath } = {}) {
  const realBin = typeof env.MATRON_CODEX_REAL_BIN === 'string' ? env.MATRON_CODEX_REAL_BIN.trim() : '';
  if (realBin && realpath(realBin)) return true;
  const shimReal = realpath(shimPath);
  for (const dir of String(env.PATH || '').split(path.delimiter).filter(Boolean)) {
    const real = realpath(path.join(dir, 'codex'));
    if (real && real !== shimReal) return true;
  }
  return false;
}
