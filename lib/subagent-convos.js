...
export function createSubagentConvoTracker({ publisher, getParentConvoId, runningStore = null, log = console } = {}) {
  ...
  function ensureChild(agentId, { label, agentType } = {}) {
    ...
    child = {
      ...
      revivePending: false,
      // Which genuine RESUME incarnation we are on (advances ONLY on a real
      // resume). Sole use is the id-less task_notification rule (#751): a
      // generation-0 run has exactly one incarnation, so an uncorrelated
      // completion is unambiguous. A corrective revival (loop #764 F1) is the
      // SAME incarnation and must NOT advance it.
      generation: 0,
      // Monotonic revival epoch: advances on EVERY successful revival (corrective
      // or genuine resume). finish()'s delivery-callback fence reads THIS, not
      // generation, so a stale `done` acknowledgement from a run that was revived
      // between its publish and its ack cannot erase the re-armed write-ahead
      // record — regardless of whether the revival was a resume or a correction.
      reviveEpoch: 0,
    };
  ...
  function _tryRevive(child, agentId, { incrementGeneration = true } = {}) {
    ...
    // The revive epoch advances on EVERY successful revival so finish()'s
    // delivery-callback fence can tell a re-armed record apart from a stale run's
    // pending ack — including a corrective revival that leaves generation put.
    child.reviveEpoch = (child.reviveEpoch ?? 0) + 1;
  ...
  function finish(agentId) {
    ...
    //
    // Fence on the revive EPOCH, not generation: the epoch advances on EVERY
    // revival (a genuine resume OR the loop #764 corrective revival), whereas
    // generation advances only on a genuine resume. A corrective revival between
    // this frame's publish and its ack must still fence this stale callback, or
    // the premature finish (the launch tool_result that ended the first run early)
    // would erase the write-ahead record the corrective revival just re-armed
    // (Codex #764 delta F1).
    const reviveEpoch = child.reviveEpoch;
    const onLocalSendComplete = runningStore
      ? () => {
        if (child.state !== CHILD_STATE_FINISHED) return;
        if (child.reviveEpoch !== reviveEpoch) return;
