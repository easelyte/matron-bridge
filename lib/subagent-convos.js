    ...
    noteBackgroundTaskStarted(toolUseId, taskId) {
      try {
        ...
        if (child && child.taskRef !== toolUseId) {
          // Loop #751 hardening: reject only a REPLAYED task_started that would
          // restore a RETIRED incarnation's ref — doing so would let a replayed
          // run-N completion pass the taskRef match and finish live run N+1,
          // defeating the completion gate.
          //
          // A ref becomes retired when the child RESUMES under a new one: run N
          // completes (child FINISHED) BEFORE the resume's task_started fires, so
          // an incoming ref-change while the child is FINISHED is a resume — record
          // the outgoing (run-N) ref as retired. A ref-change while the child is
          // RUNNING is NOT a retirement: it is a provisional FIFO mispairing being
          // corrected by its authoritative pairing (the documented
          // discovery-beats-system-event / reverse-discovery path) and must be
          // allowed. State alone is not a valid discriminator (it rejected these
          // valid corrections — delta-gate F1); retirement is tracked explicitly.
          if (child.retiredRefs?.has(toolUseId)) return;
