...
export function createSubagentConvoTracker({ publisher, getParentConvoId, runningStore = null, log = console } = {}) {
  ...
  function finish(agentId) {
    ...
    const onDelivered = runningStore
      ? () => {
        // Only clear if the child is STILL finished. A resume (revive) can land
        // between the `done` publish and its ack — removing then would discard
        // the write-ahead record of a child that is live again, leaving the
        // exact unrecoverable ghost this store exists to prevent.
