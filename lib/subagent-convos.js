    ...
    noteBackgroundTaskStarted(toolUseId, taskId) {
      try {
        ...
        if (child.taskRef === toolUseId) {
          // A task_started carrying the ref the child ALREADY holds. A genuine
          // resume always arrives under a NEW tool_use_id, so a same-ref start on
          // a FINISHED child is a REPLAY of the run that just finished — reviving
          // it would resurrect a completed child as a phantom 'running' (loop
          // #764). On a live child it is a harmless duplicate/echo.
          return child.state === CHILD_STATE_FINISHED
