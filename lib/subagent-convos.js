...
export function createSubagentConvoTracker({ publisher, getParentConvoId, runningStore = null, log = console } = {}) {
  ...
  return {
    ...
    // #751 describes (a late run-N notification killing run N+1). Only gate when
    // we have BOTH a caller-supplied tool_use_id AND a tracked ref on the child:
    // an absent tool_use_id (older streams that don't carry it) or an unknown/
    // ref-less child falls back to the prior finish-by-task_id behaviour, so a
    // legitimate completion is never dropped. finish() is already idempotent, so
    // a matching duplicate stays a safe no-op.
