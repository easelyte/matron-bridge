...
// UNLIKE edit_file — whose response is a tiny {path,bytes,mode} — read_file
// returns the file CONTENT inline, so it is the one RPC whose payload can blow
// the journal relay's 16 KiB agent_response frame cap (server-enforced;
// docs/superpowers/specs/2026-07-15-rpc-consumer-design.md). A file the bridge
// read happily but whose response the relay drops would leave the client
// hanging (no correlated failure). So read_file is bounded to the FRAME, not to
// edit_file's 5 MiB raw cap: large files belong in the file viewer / download
// path, not the inline load-then-edit editor.
//
// MAX_READ_BYTES is the raw-read budget (a fast reject before hashing);
// MAX_RESPONSE_BODY_BYTES is the authoritative bound on the SERIALIZED result
// body, checked after read so JSON escaping (a control-char-heavy file can
// expand well past its raw size) can't sneak an oversized frame past the raw
// cap. Both surface as `too_large`. The 15 KiB body budget leaves ~1 KiB
// headroom under the 16 KiB frame cap for the outer {requestId,toDeviceId,ok,
// result} envelope the publisher adds.
export const MAX_RESPONSE_BODY_BYTES = 15 * 1024;
export const MAX_READ_BYTES = MAX_RESPONSE_BODY_BYTES;
...
// not-a-file / unreadable / bad-workdir) so read_file and edit_file share ONE vocabulary;
...
export async function readFileGuarded(input, {
  ...
  // Refuse a file whose bytes don't SURVIVE a utf8 round-trip (F2). The editor
  // loads `content` as text and saves it back whole via edit_file.content — but
  // invalid-utf8 bytes decode to U+FFFD replacement chars, which re-encode to
  // DIFFERENT bytes. The sha is over the ORIGINAL bytes, so the CAS would pass
  // and a no-op load->save would silently corrupt a binary/latin1 file. Only
  // return content that edit_file can faithfully write back. (Guard at the
  // boundary — the client can't detect this after the lossy decode.)
  const text = content.toString('utf8');
  if (!Buffer.from(text, 'utf8').equals(content)) {
    throw new ReadFileError('not_text', 'file is not valid utf-8 text (would not round-trip)');
  }
  ...
  // Authoritative frame-cap guard (F3): the raw-read cap bounds bytes, but JSON
  // escaping can expand the SERIALIZED body past it, so bound the actual encoded
  // result. A response the relay would drop must fail loud here as too_large,
  // not vanish into a client timeout.
  const encodedBytes = Buffer.byteLength(JSON.stringify(body), 'utf8');
  if (encodedBytes > MAX_RESPONSE_BODY_BYTES) {
    throw new ReadFileError('too_large', `encoded response ${encodedBytes} exceeds ${MAX_RESPONSE_BODY_BYTES} bytes`);
  }
