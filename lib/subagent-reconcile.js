...
// Build the repair frames for the reconcile loop: join each stranded child
// convo id back to its persisted running-store entry so the `done` frame can
// carry the child's parentConvoId. The parentage MUST ride the frame — reconcile
// runs precisely when the original `running` upsert may never have reached the
// journal (SIGKILL before flush, socket down at mint, queue-overflow eviction),
// so `convo_upsert` may INSERT a fresh row, and parent_convo_id is written ONLY
// on INSERT (immutable after). A frame without it mints a permanent untitled
// ROOT orphan. selectStrandedChildren validated provenance, so every stranded id
// has a joinable entry — but stay defensive: parentConvoId is `undefined` for an
// unjoinable id (upsertConvoBestEffort then simply omits the key, which for the
// normal row-exists UPDATE path is a no-op anyway).
export function strandedRepairFrames(entries, stranded) {
  const byChild = new Map();
  for (const entry of entries || []) {
    if (entry && typeof entry.childConvoId === 'string' && entry.childConvoId) {
      byChild.set(entry.childConvoId, entry);
    }
  }
  return (stranded || []).map((childConvoId) => ({
    childConvoId,
    parentConvoId: byChild.get(childConvoId)?.parentConvoId,
  }));
}
