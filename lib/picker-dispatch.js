...
import { codexSessionOptions } from './codex-account.js';
...
// The /sleep confirmation card's two buttons (lib/sleep-command.js
// sleepButtons). A closed set like the pickers above rather than a shape
// match: `sleep:confirm` runs a host command that kills this process, so
// anything but these two exact values must dead-end here.
const SLEEP_ACTIONS = new Set(['confirm', 'cancel']);
...
const PICKER_VALUE = /^(model|effort|mode|timer|perm|resume|sleep):(.+)$/;
...
// record id.
function parsePickerValue(value, session) {
  ...
  if (session?.agent === 'codex' && (kind === 'model' || kind === 'effort')) {
    const options = codexSessionOptions(session);
    const offered = kind === 'model' ? options.modelOptions : options.effortLevels;
    return offered.some(option => option.value === arg) ? { kind, arg } : null;
  }
...
export function handlePickerValue(value, roomId, session, {
  ...
  const parsed = parsePickerValue(value, session);
  if (!parsed) {
    if (session?.agent === 'codex' && /^(model|effort):/.test(value)) {
      sendReply?.('Codex options have changed. Run /model or /effort again to refresh the choices.');
    }
    return false;
  }
  ...
  if (kind === 'sleep') {
    if (arg === 'confirm') confirmSleep(session, sendReply);
    else cancelSleep(session, sendReply);
    return true;
  }
  // kind === 'mode'
