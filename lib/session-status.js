...
// Module-level prior CPU snapshot. TRUE instantaneous CPU busy % comes from
// diffing tick counters against the previous sample over the wall-clock gap
// between turns — NOT loadavg, and with NO blocking sleep. The first call has
// no prior, so host_cpu is omitted that one tick (see hostVitalLimits).
let _priorCpuTicks = null;

// Instantaneous CPU busy percent (0-100 integer) since the last call, or null
// on the very first sample / a degenerate zero-elapsed window. Stateful by
// design: call it once per turn end so consecutive samples bracket real time.
...
// Host CPU + RAM as synthetic limit entries for the status frame. host_cpu is
// omitted on the first sample (no prior tick to diff against); host_ram is
// always available. Returns [] if neither can be computed.
export function hostVitalLimits() {
  const out = [];
  const cpu = sampleCpuPercent();
  if (cpu !== null) out.push({ id: 'host_cpu', label: 'CPU', percent: cpu });
  const ram = ramPercent();
  if (ram !== null) out.push({ id: 'host_ram', label: 'RAM', percent: ram });
  return out;
}
