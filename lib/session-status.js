...
export function buildSessionStatus({ model, contextTokens, contextWindow, limits, email, vitals, workdir, modelOptions, effortLevels, effort } = {}) {
  const status = {};
...
// Host CPU/RAM as SYNTHETIC LIMIT ENTRIES for the status frame's `limits`
// array (ids host_cpu / host_ram). Restored 2026-09-13: this is the surface
// every Matron client actually renders host vitals from — buildUsageMeters
// spreads `status.limits` and only OVERRIDES the live percent from the
// host-scoped ephemeral push, so without these entries no host meter exists to
// override and the CPU/RAM bars never appear (the top-level `status.vitals`
// object added by #156 is not read by any client). cpu is omitted until the
// sampler has a first valid diff; ram is computed inline. Each carries
// sampled_at_ms (#543) so the client can age a stalled gauge.
