...
export function createSubagentRunningStore({ file = DEFAULT_FILE, log = console } = {}) {
  ...
  function saveAll(data) {
    // tmp + atomic rename: a crash mid-write can't leave a truncated JSON file
    // that would then read as empty and silently forget every ghost. The tmp
    // gets an unpredictable suffix and is created O_EXCL 0600 (`wx`) so it can't
    // be pre-created / symlinked by another user between open and write — the
    // store's default path is under $HOME, but a caller may point it elsewhere.
    ...
    try {
      ...
      // A failed rename (or a write past the exclusive-create) can leave the
      // uniquely-named tmp behind; drop it so retries don't accrete orphans.
