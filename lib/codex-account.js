...
export async function withCodexAppServer(callback, {
  ...
  // These metadata/model/rate-limit queries never touch the journal, so scope
  // the full-journal read credential out of the spawned app-server's env. The
  // default `process.env` otherwise hands this short-lived child a token that
  // reads every transcript (lib/journal-cred-scope.js). Loop #750.
  const scopedEnv = stripJournalCreds(env);
  const child = spawnImpl(command, ['app-server', '--listen', 'stdio://'], {
    cwd, env: scopedEnv, stdio: ['pipe', 'pipe', 'pipe'],
  });
