...
export const PERMISSION_DECISION_TIMEOUT_MS = 1680 * 1000;
...
export function createRequestPermissionDecision(session, {
  ...
  return (toolUseId, { tool_name, expires_at } = {}) => {
    ...
    // convo -> deny immediately. Acceptance is not delivery, so bind eviction to
    // this exact pending entry and fail closed immediately if queue pressure
    // removes the card before it reaches the operator.
    const published = convoId
      ? journalPublisher.publishPermissionRequest(convoId, payload, {
        onEvicted: () => {
          if (pendingPermissionDecisions.get(key) !== pending) return;
          pending?.resolve?.({
            decision: 'deny',
