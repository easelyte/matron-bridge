...
// Published into the target session's conversation when an inbound request
// arrives for a session the idle reaper has taken down, immediately before it
// is respawned (index.js wakeSessionForInvite). Without it the user's next
// sight of that conversation is a chat request and a resumed agent with no
// account of why the session came back to life on its own.
//
// A fixed string, so unlike the formatters around it no peer-written field
// reaches it and it needs no peer-text discipline. Deliberately vague about
// WHICH agent and why — the 🤝 request notice published right after says that,
// with every interpolated field sanitised.
export const INVITE_WAKE_NOTICE =
  '⏳ This session was asleep — waking it to handle a chat request from another agent.';
