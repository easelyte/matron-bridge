...
export async function downloadAndMerge(entries, session, deps) {
  ...
  for (const { event, meta } of entries) {
    ...
    let noticeId = null;
    if (meta.msgtype === 'm.audio' && sendTranscribeNotice) {
      // Best-effort: a failed "Transcribing…" status notice must NOT abort the
      // whole burst (Phase-1 review Major 1). Guard its own send so a notice
      // rejection degrades to no-notice, not lost dispatch of this + later entries.
      try { noticeId = await sendTranscribeNotice(); }
      catch { noticeId = null; }
    }
