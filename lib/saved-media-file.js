...
// Directory-entry fsync codes to swallow: a directory fsync is genuinely
// unsupported on many filesystems/platforms and its failure is NOT data loss —
// the file's own data barrier already ran — only a weakening of the rename's
// crash-durability. Anything outside this set is left to surface.
const DIR_FSYNC_UNSUPPORTED = new Set(['EINVAL', 'ENOSYS', 'ENOTSUP', 'EOPNOTSUPP', 'EISDIR', 'EBADF', 'EACCES', 'EPERM']);

// Best-effort parent-directory fsync (secondary crash-durability barrier). Only
// swallows genuinely-unsupported codes; a storage-class failure (EIO/ENOSPC)
// still surfaces so it isn't silently reported as durable (round-2 F4).
function fsyncDirBestEffort(fsImpl, dir) {
  ...
  try {
    ...
    if (!(err && DIR_FSYNC_UNSUPPORTED.has(err.code))) throw err;
  } finally {
