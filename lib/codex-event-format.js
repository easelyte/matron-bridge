  ...
  if (event?.type === 'turn.completed') {
    if (state.pendingAgentMessage != null) {
      if (typeof ctx.runId === 'string' && ctx.runId.length > 0) {
        ...
        state.durableEvents += 1;
        ctx.publisher.publishText(
          ctx.convoId,
          { body: state.pendingAgentMessage, from: 'assistant' },
          { idemKey: `${ctx.runId}:final` },
        );
        // This records successful production into the publisher seam. T-6.2
        // will replace it with the publisher's durable-delivery acknowledgement.
        state.finalPostProduced = true;
