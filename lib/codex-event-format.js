...
export function formatAndRoute(event, ctx) {
  ...
  if (event?.type === 'item.completed' && typeof event.item?.type === 'string') {
    ...
    // a DURABLE compact card in the same family as the bash-command cards
    // instead of a raw JSON blob. tool_use_id + a humanized command label + a
    // completed status yield a clean "$ <Label>  done" card (no exit_code -> the
    // web ToolOutput badge reads "done"; no output -> no <pre> body). Uses ONLY
    // the item type and id — never any other item field — so it forwards nothing
    // the guards-first allowlist stripped, adding no egress surface. This is a
    // fallback formatter, so a future NEW item type is covered automatically
    // rather than regressing to raw JSON on the next codex schema addition.
    flushPendingAsActivity(ctx, state);
    publishDurable(ctx, state, 'publishToolOutput', {
      tool_use_id: event.item.id,
      command: humanizeItemType(event.item.type),
      status: 'completed',
    });
    return state;
  }
