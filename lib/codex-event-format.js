...
// A recognized durable item.completed that arrives missing a signature field
// signals the producer's schema drifted past what the allowlist can map (e.g. a
// future codex renames `command` or `aggregated_output`). Detecting it demotes
// the run to the diagnostic-preserving fallback (genericTextEvent still carries
// every scalar string under whatever the new field names are) instead of
// rendering a half-empty card. Unknown item TYPES are NOT a break: the allowlist
// already maps them to a bare {type, id}, a safe deliberate degradation.
//
// command_execution requires BOTH mandatory fields (`||`, not `&&`): a card that
// keeps `command` but loses the renamed output would otherwise render with a
// header + exit badge and a silently empty body — worse than an honest
// passthrough. The text-bearing items carry their body under one of two keys
// (text OR summary), so only a break when NEITHER is present (`&&`).
...
//
// The key set is a STRICT allowlist on purpose, NOT "every scalar string on the
// item". The shared redactor is KEY-AWARE — it recognizes a secret only when it
// sees `apiKey: <value>` as text — but this pipeline redacts each field VALUE in
// isolation (redactStrings) before serializing, so a structural JSON key never
// reaches the key-aware rules. Copying arbitrary item fields would therefore let
// a field literally named `api_key`/`token` egress its plaintext value into the
// journal (its bare value alone is not token-shaped, so value-only detection
// misses it). We accept that a drifted schema's RENAMED command/output field is
// not preserved by name — the drift is still DETECTED (eventShapeBreaksSchema)
// and WARNED, and the item's type/id plus any allowlisted diagnostic key survive.
// Preserving renamed scalars safely would require key-aware structural redaction
// (redact the serialized JSON, not each value) — a separate, tested change.
...
function textOf(item) {
  // Prefer the first field with NON-EMPTY content: an empty `text` beside a real
  // `summary` must not win and blank out the message (an empty durable :final
  // would hide the real answer and falsely report successful delivery).
  if (isNonEmptyString(item?.text)) return item.text;
  if (isNonEmptyString(item?.summary)) return item.summary;
  // Both present-but-empty (or one empty, other absent): keep the empty string
  // rather than serializing the identity object into a bogus body.
  if (typeof item?.text === 'string') return item.text;
  if (typeof item?.summary === 'string') return item.summary;
  return stringify(item);
}

// Whether an agent_message carries a usable answer body. The SINGLE predicate
// gating final-answer candidacy on BOTH the rich and the demoted paths: an
// empty/whitespace-only message must never be buffered as the final, or
// finalizeTurn would publish blank content under `${runId}:final` and falsely
// report a delivered final answer.
function hasUsableAgentBody(item) {
  return isNonEmptyString(item?.text) || isNonEmptyString(item?.summary);
}
...
function flushPendingNarration(ctx, state) {
  ...
  // Deterministic idempotency key so a bridge restart that reattaches to a live
  // run and replays the transcript from the start re-derives the SAME key and
  // the journal dedupes it (matching the final answer's `${runId}:final`).
  // Without it, safePublish assigns a fresh UUID per replay and each narration
  // line duplicates. The key is a per-run monotonic narration counter, NOT the
  // codex item id: the id reaches here already redacted (a random per-process
  // marker if it matched an operator pattern → unstable across restarts), and
  // the producer contract does not guarantee id uniqueness. Replay is
  // order-deterministic (same transcript, same sequence), so the Nth narration
  // flush gets the same counter every time. Best-effort no key when runId absent.
  state.narrationSeq = (state.narrationSeq ?? 0) + 1;
  const options = (typeof ctx.runId === 'string' && ctx.runId)
    ? { idemKey: `${ctx.runId}:narration:${state.narrationSeq}` }
    : undefined;
  publishDurable(ctx, state, 'publishText', { body, from: 'assistant' }, options);
}

// Terminal handling for turn.completed. Retains the buffered final answer under
// the stable `${runId}:final` key, marks the turn seen, and idles the convo.
// Extracted so it can also run when a run has been demoted to passthrough
// mid-stream (schemaShapeBroken): the shape-drift latch suppresses item CARD
// rendering, but the run still owns its final answer and completion signal —
// dropping them would strand the buffered message and leave the convo spinning.
function finalizeTurn(ctx, state) {
  if (state.pendingAgentMessage != null) {
    if (typeof ctx.runId === 'string' && ctx.runId.length > 0) {
      ...
      const enqueued = ctx.publisher.publishText(ctx.convoId, payload, publishOptions);
...
export function formatAndRoute(event, ctx) {
  ...
  if (!richRenderingEnabled(ctx, state)) {
    ...
    // A run demoted MID-STREAM (shape-drift latch) still owns its terminal
    // signal and may see a real final answer AFTER demotion. A from-start-
    // ineligible run (schemaShapeBroken never set) never buffered anything and
    // keeps its unchanged raw passthrough.
    if (state.schemaShapeBroken) {
      if (event?.type === 'turn.completed') {
        // Retain the LATEST buffered answer (see below) under `${runId}:final`
        // and idle the convo, rather than stranding it as a raw JSON line.
        return finalizeTurn(ctx, state);
      }
      // A post-demotion agent_message with a USABLE textual body is the new
      // final-answer candidate. The usable-text guard matters: a malformed
      // agent_message (text/summary both renamed away — the very drift that
      // latched the run) would otherwise be handed to textOf, which serializes
      // the identity object into a bogus `{"type":"agent_message","id":...}`
      // durable :final. Flush any earlier buffered message as narration first
      // (this one supersedes it), so a stale pre-demotion message is never
      // resurrected as the final while the true last answer is dropped to text.
      const isUsableAgentMessage = event?.type === 'item.completed'
        && event.item?.type === 'agent_message'
        && hasUsableAgentBody(event.item);
      if (isUsableAgentMessage) {
        flushPendingNarration(ctx, state);
        state.pendingAgentMessage = textOf(event.item);
        return state;
      }
      // Any other durable event (including a malformed agent_message with no
      // usable text) supersedes buffered narration → flush it, then pass this
      // event through raw rather than fabricating a final answer from identity.
      flushPendingNarration(ctx, state);
    }
  ...
  if (event?.type === 'item.completed' && event.item?.type === 'agent_message') {
    flushPendingNarration(ctx, state);
    // Only a message with a usable (non-empty) body becomes the final candidate
    // — same predicate as the demoted branch. An empty/whitespace-only message
    // is passed through raw rather than buffered, so it can't blank the :final.
    if (hasUsableAgentBody(event.item)) {
      state.pendingAgentMessage = textOf(event.item);
      return state;
    }
    passThrough(event, ctx, state);
