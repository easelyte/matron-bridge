...
// Schema-skew fallback: keep identity plus human-readable diagnostic content.
// At top level only a small set of known diagnostic keys is kept (a codex event
// carries no free-form top-level scalars). But on `item` we CANNOT know which
// key now holds the command/output/answer under a drifted schema, so every
// scalar STRING field is preserved — losing the field-name mapping is
// acceptable, silently dropping the diagnostic text is not. Nested objects and
// arrays are still dropped (never publish arbitrary new structure), and every
// preserved value passes through the shared redactor downstream.
function genericTextEvent(event) {
  ...
  if (event?.item && typeof event.item === 'object') {
    ...
    for (const [key, value] of Object.entries(event.item)) {
      if (typeof value === 'string') fallback.item[key] = value;
    }
  }
  return fallback;
}
...
function flushPendingNarration(ctx, state) {
  ...
  const { body, id } = state.pendingAgentMessage;
  state.pendingAgentMessage = null;
  // Deterministic idempotency key (runId + the codex item id) so a bridge
  // restart that reattaches to a live run and replays the transcript from the
  // start re-derives the SAME key and the journal dedupes it — matching the
  // final answer's `${runId}:final`. Without it, safePublish assigns a fresh
  // UUID per replay and each narration line duplicates. Falls back to no key
  // (best effort) when either id is unavailable.
  const options = (typeof ctx.runId === 'string' && ctx.runId && typeof id === 'string' && id)
    ? { idemKey: `${ctx.runId}:msg:${id}` }
    : undefined;
...
function finalizeTurn(ctx, state) {
  if (state.pendingAgentMessage != null) {
    if (typeof ctx.runId === 'string' && ctx.runId.length > 0) {
      ...
      state.durableEvents += 1;
      const payload = { body: state.pendingAgentMessage.body, from: 'assistant' };
...
export function formatAndRoute(event, ctx) {
  ...
  if (!richRenderingEnabled(ctx, state)) {
    ...
    // A run demoted MID-STREAM (shape-drift latch) may have buffered a final
    // answer under the rich path before it tripped, and still owns its terminal
    // signal. Route turn.completed through the terminal handler so that answer
    // is retained and the convo idles, rather than stranding it and passing the
    // lifecycle event through as a raw JSON line. A from-start-ineligible run
    // (schemaShapeBroken never set) never buffered anything → unchanged
    // passthrough.
    if (state.schemaShapeBroken && event?.type === 'turn.completed') {
  ...
  if (event?.type === 'item.completed' && event.item?.type === 'agent_message') {
    flushPendingNarration(ctx, state);
    state.pendingAgentMessage = { body: textOf(event.item), id: event.item.id };
    return state;
  }
