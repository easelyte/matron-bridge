...
// Classifies untrusted text as an assignment-dominated env dump. `threshold` is
// the minimum number of assignment lines: command OUTPUT legitimately prints
// multi-line KEY=value blocks (git config, dotenv echoes), so it keeps the
// 3-line bar to avoid false drops, while forwarded DIAGNOSTIC text (top-level
// error strings) is rarely two bare assignments and uses a stricter 2-line bar.
function looksLikeRawEnvDump(output, threshold = 3) {
  ...
  return assignments >= threshold && assignments / lines.length >= 0.6;
}
...
// an env dump under the STRICTER 2-line diagnostic bar — the caller then drops
// the whole event, so a small dump can never egress via a top-level error
// string under an innocuous env name the baseline redactor cannot catch.
function copySafeTextualFields(source, target) {
  if (!source || typeof source !== 'object') return true;
  for (const key of SAFE_TEXTUAL_KEYS) {
    const value = source[key];
    if (typeof value !== 'string') continue;
    if (looksLikeRawEnvDump(value, 2)) return false;
    target[key] = value;
  }
  return true;
}
