...
export function redactAndRoute(event, ctx) {
  ...
  let redactedMeta;
  try {
    const pinnedSchema = schemaIsPinned(ctx.meta?.schemaVersion);
    const allowed = pinnedSchema ? allowlistedEvent(event) : genericTextEvent(event);
    if (allowed === null) {
  ...
  if (event?.type === 'turn.completed') {
    if (state.pendingAgentMessage != null) {
      if (typeof ctx.runId === 'string' && ctx.runId.length > 0) {
        ...
        // Socket-write completion is not a durable journal acknowledgement.
        // Keep the retained payload available for idempotent reconnect repair
        // and leave finalPostLanded unknown until an ack protocol exists.
        const publishOptions = { idemKey: `${ctx.runId}:final` };
