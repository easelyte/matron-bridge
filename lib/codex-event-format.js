...
// reasoning, and the lifecycle events) has been stable across the 0.146.x–
// 0.155.x codex-cli line, so we accept a compatibility BAND instead of an exact
// pin. An exact-equality pin silently raw-dumps every other version present in a
// mixed fleet (observed live: 0.146.0, 0.146.1, 0.147.0 and 0.155.1) and
// re-breaks on each codex upgrade. Versions outside [MIN, MAX_EXCLUSIVE) still
// fail safe to the text-passthrough fallback (raw JSON, never a redaction
// bypass), so widening the band is a low-risk cosmetic fix.
//
// This band still needs a manual bump on each codex major upgrade. A
// version-tolerant routing that keys on event shape rather than the version
// string is deferred to its own change — the redaction egress guards are gated
// on the item.completed envelope, so generalizing routing without first making
// those guards unconditional across every field/envelope reopens a secret-
// egress hole (proven by adversarial review, 2026-09-21). Track: version-
// tolerant codex-viz rendering (loop filed 2026-09-21).
const MIN_SUPPORTED_CODEX_VERSION = [0, 146, 0];
const MAX_SUPPORTED_CODEX_VERSION_EXCLUSIVE = [0, 156, 0];
