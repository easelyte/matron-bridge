...
// Replace the VALUE on any line that is itself a bare `NAME=...` assignment
// (env-dump shape) with a fixed marker, leaving prose and inline `key=value`
// mentions inside a sentence untouched. Applied to newly forwarded DIAGNOSTIC
// text (top-level error strings) so an env dump under an innocuous env name the
// baseline redactor cannot catch never egresses — regardless of how many
// non-assignment lines dilute it below any ratio heuristic (Codex review r3 F1).
// Handles both newline- and NUL-delimited dumps.
