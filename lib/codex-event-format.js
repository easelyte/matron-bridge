...
function parseCodexSchemaVersion(schemaVersion) {
  ...
  // Reject overflow/unsafe components: a very long digit run parses to
  // Infinity, which the floor-only comparison would wrongly treat as supported
  // and route richly instead of failing safe to text passthrough. The old
  // upper-bound check happened to reject Infinity; the floor-only gate must
  // validate it explicitly (Codex review r3 F2 / P8 guard boundary inputs).
...
// Classifies command OUTPUT as an assignment-dominated env dump (>=3 lines,
// >=60% assignments). Command output legitimately prints multi-line KEY=value
// blocks (git config, dotenv echoes), so the 3-line bar avoids false drops;
// forwarded DIAGNOSTIC text is protected differently (scrubAssignmentValues),
// so it does not rely on this ratio heuristic.
...
// Replace the VALUE of every bare `NAME=...` assignment entry (env-dump shape)
// with a fixed marker, leaving prose and inline `key=value` mentions inside a
// sentence untouched. Applied to newly forwarded DIAGNOSTIC text (top-level
// error strings, fallback diagnostics) so an env dump under an innocuous env
// name the baseline redactor cannot catch never egresses — regardless of how
// many non-assignment entries dilute it below any ratio heuristic.
//
// NUL is the AUTHORITATIVE record separator when present (printenv --null,
// /proc/self/environ): a single environment VALUE may itself contain newlines,
// so in that mode newline must NOT be treated as an entry boundary or the
// continuation bytes of a secret value leak past the scrub (Codex delta gate).
// Newline framing is used only when no NUL is present.
function scrubAssignmentValues(text) {
  const nulFramed = text.includes('\0');
  // In NUL mode split on NUL only (a value may contain any newline variant).
  // Otherwise split on EVERY line-boundary variant — LF, CRLF, lone CR,
  // vertical tab, form feed, NEL, LS, PS — so no delimiter variant can leave an
  // assignment unanchored and leak its value (Codex terminal gate: lone CR).
  const separators = nulFramed ? /\0/ : /\r\n|[\n\r\v\f\u0085\u2028\u2029]/;
  // In NUL mode a record may ALSO open with prose and carry an assignment on a
  // later line (`fatal\nALPHA=secret\0…`, loop #787 Codex F1): an assignment
  // at ANY line start inside the record redacts from there to the record end
  // (a value may span lines, so the rest of the record is value bytes).
  const assignment = nulFramed
    ? /(^|\r\n|[\n\r\v\f\u0085\u2028\u2029])([ \t]*[A-Za-z_][A-Za-z0-9_]*=)[\s\S]*$/
    : /^()([ \t]*[A-Za-z_][A-Za-z0-9_]*=)[\s\S]*$/;
  return text
    .split(separators)
    .map(entry => entry.replace(assignment, '$1$2[REDACTED-ENV]'))
    .join(nulFramed ? '\0' : '\n');
}
...
// scrubbing the value of every bare `NAME=...` assignment line first (they are
// also redacted downstream). Scrubbing rather than a dominance-ratio drop means
// a small env dump cannot egress via a diagnostic string no matter how much
// prose dilutes it, while genuine prose diagnostics are preserved.
function copySafeTextualFields(source, target) {
  if (!source || typeof source !== 'object') return;
  for (const key of SAFE_TEXTUAL_KEYS) {
    const value = source[key];
    if (typeof value !== 'string') continue;
    target[key] = scrubAssignmentValues(value);
  }
}
...
function allowlistedEvent(event) {
  ...
  // survive instead of being silently dropped by the item filter; assignment
  // values in them are scrubbed so a dump cannot ride out on a diagnostic.
  copySafeTextualFields(event, allowed);
  // `turn.failed` nests its diagnostic as `{error: {message}}`; lift ONLY that
  // string (scrubbed like every other diagnostic) so the failure line can say
  // why, instead of the bare stub (loop #787).
  if (type === 'turn.failed' && typeof allowed.message !== 'string'
      && typeof event?.error?.message === 'string') {
    allowed.message = scrubAssignmentValues(event.error.message);
  }
  ...
  if (event?.item && typeof event.item === 'object') {
    ...
    // An unknown/future item type forwards ONLY its { type, id } stub — never
    // its arbitrary fields. Preserving a newer item's textual fields would
    // reopen an egress surface for a future codex that renames a command item
    // and streams output in a textual field the command guard never sees
    // (Codex review r1/r2). Diagnostics degrade safely to the type stub; the
    // final answer still lands via the agent_message path.
...
// human-readable diagnostic/answer fields. Bare `NAME=...` assignment values
// are scrubbed the SAME way as the supported-schema diagnostic path, so an env
// dump cannot egress via a below-floor/malformed-version diagnostic either
// (Codex delta gate). Values also pass through the shared redactor; arbitrary
// new object structure is never published.
function genericTextEvent(event) {
  ...
  for (const [key, value] of Object.entries(event || {})) {
    if (textualKeys.has(key) && typeof value === 'string') fallback[key] = scrubAssignmentValues(value);
  }
  if (event?.item && typeof event.item === 'object') {
    fallback.item = {};
    ...
    for (const [key, value] of Object.entries(event.item)) {
      if (textualKeys.has(key) && typeof value === 'string') fallback.item[key] = scrubAssignmentValues(value);
    }
  }
  return fallback;
}
...
// Human-readable label for an otherwise-unrecognized codex item type, used by
// the generic item.* fallback so a new item type (web_search, mcp_tool_call, …)
// renders as a titled line/activity instead of a raw JSON blob.
// `web_search` -> "Web search", `mcp_tool_call` -> "Mcp tool call". Operates on
// the type string ONLY (already redacted, and the only field the guards-first
// allowlist forwards for an unknown item); it never reaches into other fields.
//
// SANITIZE-FOR-THE-SINK: the result is interpolated into an assistant Markdown
// body (and a plain activity detail), so it is reduced to a bounded
// [A-Za-z0-9 ] grammar FIRST — every other byte (backtick, newline, [](), bidi
// control, …) collapses to a single space. A hostile or newer producer's item
// type therefore cannot escape the inline-code wrapper to inject links or
// spoofed instructions (Codex terminal review: Markdown-injection sink). The
// label is length-capped so an oversized type cannot flood the journal.
function humanizeItemType(type) {
  if (typeof type !== 'string') return 'Tool';
  const spaced = type.replace(/[^A-Za-z0-9]+/g, ' ').trim().slice(0, 80).trim();
  if (spaced.length === 0) return 'Tool';
...
// A codex diagnostic interpolated into an assistant Markdown body. Same
// sanitize-for-the-sink rule as humanizeItemType: the text lands inside an
// inline code span, so backticks are neutralized (it cannot close the span and
// inject links/instructions), every control/whitespace run (newlines, bidi
// controls) collapses to one space, and the length is bounded.
const MAX_DIAGNOSTIC_CHARS = 500;
function diagnosticSpan(value) {
  ...
  const bounded = flat.length > MAX_DIAGNOSTIC_CHARS
    ? `${flat.slice(0, MAX_DIAGNOSTIC_CHARS - 1)}…`
    : flat;
  return `\`${bounded}\``;
}

function diagnosticOf(event) {
...
function emitDurableFinalAnswer(ctx, state) {
  ...
  if (typeof ctx.runId === 'string' && ctx.runId.length > 0) {
    ...
    const enqueued = ctx.publisher.publishText(ctx.convoId, payload, {
      ...
      onLocalSendComplete: () => ctx.markFinalAnswerDelivered?.(ctx.runId),
    });
...
export function formatAndRoute(event, ctx) {
  ...
  // Loop #787: known codex exec --json protocol events that used to fall to
  // the raw-JSON passthrough on the supported path. A top-level `error` and a
  // `turn.failed` become readable warning lines (the diagnostic already passed
  // the allowlist scrub + redactor); `item.updated` (todo_list progress) is
  // transient state, so it is an ephemeral activity, never a durable post.
  if (event?.type === 'error' && !event.item) {
    flushPendingAsActivity(ctx, state);
    const span = diagnosticSpan(diagnosticOf(event));
    publishDurable(ctx, state, 'publishText', {
      body: span ? `⚠️ Codex error: ${span}` : '⚠️ Codex error',
      from: 'assistant',
    });
    return state;
  }
  ...
  if (event?.type === 'turn.failed') {
    // Keep real assistant text: an answer the turn produced before failing
    // still lands as the durable final post, then the failure line.
    emitDurableFinalAnswer(ctx, state);
    const span = diagnosticSpan(diagnosticOf(event));
    publishDurable(ctx, state, 'publishText', {
      body: span ? `⚠️ Codex turn failed: ${span}` : '⚠️ Codex turn failed',
      from: 'assistant',
    });
    state.terminalSeen = true;
    ctx.publisher.publishActivity(ctx.convoId, 'idle');
  ...
  if (event?.type === 'item.updated' && typeof event.item?.type === 'string') {
    ctx.publisher.publishActivity(ctx.convoId, 'tool', humanizeItemType(event.item.type));
    return state;
  }
  ...
  if (event?.type === 'item.started') {
    ...
    } else if (typeof event.item?.type === 'string') {
      // GENERIC item.started fallback (loop #772): any recognized-shape item
      // whose type we don't render specially (web_search, mcp_tool_call, …)
      // still shows an ephemeral "tool" activity indicator — like
      // command_execution/file_change do — instead of dumping raw JSON into the
      // journal. Uses ONLY the item type (label), never other item fields, so it
      // adds no egress surface. A future NEW item type is covered automatically:
      // this is a fallback formatter, not a per-type allowlist.
      ctx.publisher.publishActivity(ctx.convoId, 'tool', humanizeItemType(event.item.type));
    } else {
      // Truly unstructured (no string item.type) — keep the raw text passthrough.
      passThrough(event, ctx, state);
    }
    return state;
  }
  ...
  if (event?.type === 'item.completed' && typeof event.item?.type === 'string') {
    // GENERIC item.completed fallback (loop #772): any recognized-shape item
    // whose type we don't render specially (web_search, mcp_tool_call, …) lands
    // a DURABLE, compact, FORMATTED line naming the completed tool step instead
    // of a raw JSON blob. Uses ONLY the humanized item type — never the id or any
    // other item field — so nothing the guards-first allowlist stripped can
    // egress. A future NEW item type is covered automatically; this is a
    // fallback formatter, not a per-type allowlist.
    //
    // Deliberately NOT a publishToolOutput card: the web/apple clients render an
    // exit_code-less tool_output with a success ("done") badge, but a
    // status-bearing item (e.g. an mcp_tool_call that completed with
    // status "failed") has had its status stripped by allowlistedEvent, so the
    // bridge cannot honestly assert success — a "done" card would misreport a
    // failed tool as succeeded (Codex F1). A neutral text line makes no
    // success/failure claim. (Honoring a real terminal status end-to-end is a
    // cross-component client-contract change tracked separately — Codex F2, and
    // it also affects the existing file_change card.)
    flushPendingAsActivity(ctx, state);
    publishDurable(ctx, state, 'publishText', {
      body: `\`${humanizeItemType(event.item.type)}\``,
      from: 'assistant',
    });
    return state;
  }
