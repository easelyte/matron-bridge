  ...
  if (!richRenderingEnabled(ctx, state)) {
    ...
    if (state.schemaShapeBroken) {
      ...
      if (event?.type === 'item.completed' && event.item?.type === 'agent_message') {
        // A recognizable agent_message after demotion is the new final-answer
        // candidate. Flush any earlier buffered message as narration first (it
        // was superseded by this one), so a STALE pre-demotion message can never
        // be resurrected as the final answer while the true last answer is
        // dropped to raw text.
        flushPendingNarration(ctx, state);
        state.pendingAgentMessage = textOf(event.item);
        return state;
      }
      // Any other durable event supersedes buffered narration → flush it as
      // narration so it is not stranded, then pass this event through raw.
      flushPendingNarration(ctx, state);
    }
