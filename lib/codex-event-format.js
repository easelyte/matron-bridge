...
function isRawEnvDumpCommand(command) {
  ...
  if (/^(?:\/usr\/bin\/)?(?:env|printenv)$/.test(trimmed)) return true;
  return /^(?:\/bin\/)?(?:ba)?sh\s+-lc\s+(['"])(?:\/usr\/bin\/)?(?:env|printenv)\1$/.test(trimmed);
}
...
function allowlistedEvent(event) {
  ...
  if (itemType === 'command_execution') {
    if (type === 'item.completed' && isRawEnvDumpCommand(event.item.command)) return null;
