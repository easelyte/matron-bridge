...
export function planPrintModelSwitch(session, arg, { hasInflightMedia = false } = {}) {
  if (!isValidModelArg(arg)) {
    return { ok: false, message: `Unknown model "${arg}". Try: ${VALID_ALIAS_HINT} (or a full claude-* name).` };
  }
  if (session.busy) {
    // Not a refusal any more: hand back a defer signal (with the normalized
    // alias) so the caller can park `!model <alias>` on the deferred-command
    // stash. The turn-end seam replays it BEFORE the queue flush — the
    // replay recreates the session with the new model and the carried queue
    // flushes on the replacement — so the switch applies ahead of every
    // queued message, /compact included. Validation stays above: an unknown
    // alias is refused outright, busy or not, and never parked.
    return {
      ok: false,
      defer: true,
      normalized: normalizeModelArg(arg),
      message: `🧠 Queued /model ${aliasLabel(arg)} — it will apply as soon as this turn finishes, before any queued messages.`,
    };
  }
  ...
  // A print-mode /model switch recreates the `claude -p` process (same teardown
  // as /switch / /restart / /mode). Refuse while a journal-media attachment for
  // this conversation is still being prepared — the teardown would make the
  // media router drop it at its delivery-time canonicality guard. Checked after
  // the busy-defer, so a mid-turn switch still parks (and by the turn-end replay
  // the age-bounded prep window has almost always drained); it only blocks a
  // switch that would otherwise recreate the session right now.
